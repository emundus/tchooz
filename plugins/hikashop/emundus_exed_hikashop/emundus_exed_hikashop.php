...
class PlgHikashopEmundus_exed_hikashop extends CMSPlugin
{
	...
	public function onAfterCartProductsLoad(&$cart)
	{
		...
		$element_totalite = !empty($element_totalite_id) ? $h_files->getFabrikElementData($element_totalite_id) : [];
		$element_discount = !empty($element_discount_id) ? $h_files->getFabrikElementData($element_discount_id) : [];
		...
		if (!empty($cart_id) && !empty($fnum))
		{
			...
			if ($status == $discount_payment_status)
			{
				...
				if (!empty($cart_products))
				{
					...
					if ($type == 'variant')
					{ // if the product is a variant, then cart is surely already updated
						return;
					}

					if (!empty($element_totalite))
					{
						...
						Log::add('User ' . $user_id . ' with fnum ' . $fnum . ' has no totalite prices found', Log::INFO, 'com_emundus.emundus_exed_hikashop_plugin');
					}

					if (!empty($element_discount))
					{
						...
						Log::add('User ' . $user_id . ' with fnum ' . $fnum . ' has no discount price found', Log::INFO, 'com_emundus.emundus_exed_hikashop_plugin');
					}
					...
					try
					{
						...
						if (!empty($total_applicant_price))
						{
						...
						if (!empty($discounted_applicant_price))
						{
						...
						$default_variant_to_show = !empty($variant_totalite_product_id) ? $variant_totalite_product_id : (!empty($variant_discounted_product_id) ? $variant_discounted_product_id : $parent_product_id);

						if ($default_variant_to_show != $parent_product_id)
						{
							$updated_cart = $m_payment->updateHikashopCart($cart_id, [$default_variant_to_show]);

							if (is_null($updated_cart))
							{
								Log::add('User ' . $user_id . ' with fnum ' . $fnum . ' tried to update the cart with id ' . $cart_id . ' with the product ' . $default_variant_to_show, Log::ERROR, 'com_emundus.emundus_exed_hikashop_plugin');
								$app->enqueueMessage(Text::_('COM_EMUNDUS_ERROR_PRODUCT_NOT_FOUND'), 'error');
								$app->redirect('/');
							}
							else
							{
								Log::add('User ' . $user_id . ' with fnum ' . $fnum . ' updated the cart with id ' . $cart_id . ' with the product ' . $default_variant_to_show, Log::INFO, 'com_emundus.emundus_exed_hikashop_plugin');
							}
						}
					}
					catch (Exception $e)
