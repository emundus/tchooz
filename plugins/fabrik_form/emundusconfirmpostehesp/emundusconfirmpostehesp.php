<?php
/**
 * @version 2: emundusconfirmpostehesp 2020-09-28 Benjamin Retord
 * @package Fabrik
...
class PlgFabrik_FormEmundusconfirmpostehesp extends plgFabrik_Form
{
	/**
	 * Status field
	 *
	...
	public function onAfterProcess() {
		...
		$status_actuel = explode(',',$status_actuel);
        $status_wanted_after_sent = explode(',',$status_wanted_after_sent);

        $status = array_combine($status_actuel, $status_wanted_after_sent);

        $status_after_sent = $status[$status_by_fnum];

		$query = 'UPDATE #__emundus_campaign_candidature SET submitted=1, date_submitted=NOW(), status='.$status_after_sent.' WHERE applicant_id='.$student->id.' AND campaign_id='.$student->campaign_id. ' AND fnum like '.$db->Quote($student->fnum);
		$db->setQuery($query);
