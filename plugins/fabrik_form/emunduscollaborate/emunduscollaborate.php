...
class PlgFabrik_FormEmundusCollaborate extends plgFabrik_Form
{
	...
	public function onBeforeLoad()
	{
		...
		if(empty($key)) {
			$this->app->enqueueMessage(Text::_('PLG_FABRIK_FORM_EMUNDUSCOLLABORATE_ERROR_KEY'), 'error');
		...
		if($collaboration->uploaded == 0) {
			...
			if(empty($shared_user_id)) {
				...
				if($accepted) {
					...
					if($this->user->guest) {
						$this->app->enqueueMessage(Text::_('PLG_FABRIK_FORM_EMUNDUSCOLLABORATE_ACCOUNT_ALREADY_EXIST'), 'success');
						$this->app->redirect(Route::_($redirect_url));
					} else {
			...
			$this->app->enqueueMessage(Text::_('PLG_FABRIK_FORM_EMUNDUSCOLLABORATE_ERROR_ALREADY_UPLOADED'), 'error');
			$this->app->redirect('index.php');
		}
