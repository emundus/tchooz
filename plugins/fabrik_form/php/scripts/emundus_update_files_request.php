...
if (!empty($existingIds)) {
    if(count($existingIds) > 1){
      try{
        ...
        JLog::add('Failed to delete request for id ' . $existingIds[array_key_last($existingIds)] . ' '. $e->getMessage(), Log::ERROR, 'com_emundus.error');
      }
    ...
    try{
      ...
      $db->execute();
        ...
        JLog::add('Failed to update repeat request for id ' . $existingIds[0] . ' '. $e->getMessage(), Log::ERROR, 'com_emundus.error');
    }
    ...
    try{
      ...
      $db->execute();
        ...
        JLog::add('Failed to update repeat request for id ' . $lastIds . ' '. $e->getMessage(), Log::ERROR, 'com_emundus.error');
    }
