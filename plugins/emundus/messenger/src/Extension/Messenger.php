	...
	public function prepareNotifications(GenericEvent $event): void
	{
		...
		if ($send_notifications)
		{
			...
			else
			{
				...
				// Add users to notify directly associated to the file's
				if ($messenger_notify_users_programs == 1)
				{
					...
					$users_to_send = array_filter(array_unique(array_merge($users_to_send, $users_associated)));
				}

				// Add users to notify by their group set in the config
				...
				// Add users to notify manually set in the config
				...
				if (!empty($users_to_send))
				{
					...
					$messengerAction = $actionRepository->getByName('messenger');

					// Just to be sure, check for every user in the list if they can access the file
					foreach ($users_to_send as $key => $user_to_check)
					{
						$can_access = \EmundusHelperAccess::asAccessAction($messengerAction->getId(), CrudEnum::CREATE->value, $user_to_check, $data['fnum']);
