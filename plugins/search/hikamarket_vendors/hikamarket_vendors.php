...
class plgSearchHikamarket_vendors extends JPlugin {
	...
	public function onSearch($text, $phrase = '', $ordering = '', $areas = null) {
		...
		$select = ' vendor.vendor_id AS id'.
			', vendor.vendor_name AS title'.
			', vendor.vendor_description AS text'.
			', vendor_created as created'.
		...
		$unicodeslugs = $jconfig->get('unicodeslugs');
		$lang = JFactory::getLanguage();

		foreach($rows as $k => $row) {
			if(!empty($row->vendor_alias)) {
			...
			if($urlSafe) {
				$alias = $app->stringURLSafe(strip_tags($alias));
			} else {
				$alias = JFilterOutput::stringURLSafe(strip_tags($alias));
			}
