...
-- Los plugins de integracion de Track Actions (Akeeba Backup, Acymailing, Securitycheck Pro, Control Center)
-- insertan su propia extension por VALOR, con su propio id autoincremental. Como esta tabla nunca tuvo una
-- clave unica en `extension`, una extension pudo quedar duplicada con ids distintos. Lo limpiamos antes de
-- insertar nada mas y anadimos la clave unica para que no vuelva a ocurrir.
DELETE t1 FROM `#__securitycheckpro_trackactions_extensions` t1
INNER JOIN `#__securitycheckpro_trackactions_extensions` t2
    ON t1.extension = t2.extension AND t1.id > t2.id;
ALTER TABLE `#__securitycheckpro_trackactions_extensions` ADD UNIQUE KEY `idx_extension` (`extension`);
