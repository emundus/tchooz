...
class ProtectionModel extends BaseDatabaseModel
{
   ...
   /**
    ...
    public function load(string $keyName): ?string
    {
        ...
        // 1) Validaci�n: evita claves malformadas/excesivas
        ...
        // 2) Construcci�n segura con protecci�n nativa (sin bind)
        $query
        ...
        try {
            ...
            // 3) Decodificaci�n segura
            $data = json_decode($json, true, 512, JSON_THROW_ON_ERROR);
            if (!is_array($data)) {
                return null;
            }

            // Mant�n compatibilidad con $this->config si lo usas en otros sitios
    ...
    /**
	 * Guarda la configuraci�n en la tabla de almacenamiento.
	 *
	 * @param  string $key_name  Clave bajo la que se persiste la configuraci�n
	 * @return void
	 *
	 * @throws InvalidArgumentException Si la clave no cumple el patr�n permitido
	 * @throws RuntimeException         Si falla el guardado o la codificaci�n JSON
	 */
	public function save(string $key_name): void
	{
		// Asegura que la config est� cargada
		if ($this->config === null) {
			$this->load($key_name);
		}

		// 1) Validaci�n estricta de la clave:
		//    - Letras, n�meros, punto, guion, guion bajo, dos puntos
		//    - Longitud m�xima 128
		...
		// 2) Normaliza y serializa la configuraci�n a JSON de forma segura
		...
		// 3) Transacci�n + upsert manual: UPDATE ? si no afecta, INSERT
		try {
			$db->transactionStart();
   ...
   /**
    ...
    function saveConfig($newParams, $key_name = 'cparams')
    {
   	...
   	/**
	 * Comprueba si un fichero existe en la ra�z del sitio de forma segura.
	 *
	 * @param  string $filename  Ruta relativa (respecto a JPATH_SITE) del fichero a comprobar
	 * @return bool              TRUE si existe, FALSE en caso contrario
	 *
	 * @throws InvalidArgumentException Si la ruta es inv�lida o apunta fuera de JPATH_SITE
	 */
	public function existsFile(string $filename): bool
	{
		...
		// Validamos que la ruta est� dentro de JPATH_SITE
	 ...
	 * @return bool         TRUE si la copia se realiz�, FALSE si no exist�a el .htaccess
	 *
	 * @throws InvalidArgumentException Si el nombre de backup es inv�lido
	...
	public function makeBackup(string $name): bool
	{
		...
		// 2) Validaci�n estricta del nombre de backup
		//    Solo letras, n�meros, guion, guion bajo y punto. M�x. 64 caracteres.
	...
	/**
	 * Modifica los valores del array 'ConfigApplied' seg�n las opciones detectadas en el .htaccess actual.
	 *
	...
	public function getConfigApplied(): array
	{
		 ...
		 * - block: etiqueta del bloque Begin/End (si existe para esa opci�n)
		 * - rules: regex equivalentes que consideramos �aplicado� aunque est�n fuera del bloque
		 */
		$detectors = [
			'prevent_access' => [
			...
			'prevent_unauthorized_browsing' => [
				// No tiene etiqueta �nica siempre; detectamos bloque si lo tuvieras:
			...
			'redirect_to_www' => [
				// No siempre hay etiqueta �nica, pero muchas veces a�ades �Securitycheck Pro Redirect ��
		...
		// 1) Detecci�n por bloque/reglas para TODAS las opciones del mapa
		...
		// 2) Detectores din�micos desde configuraci�n del usuario (escapados)
		...
		// own_code: cada l�nea/bloque debe estar incluido literalmente (case-insensitive)
	...
	/**
	 * Convierte texto multil�nea en array de l�neas no vac�as, recortadas.
	 *
	 * @return list<string>
	 */
	...
	public function protect(): bool
	{
		...
		$sanitizeHeaderValue = static function (string $v, int $maxLen = 4096): string {
			// Elimina CR/LF/NUL para evitar inyecci�n de l�neas/directivas
			$v = str_replace(["\r", "\n", "\0"], '', $v);
			// Colapsa espacios repetidos
			$v = preg_replace('/[ \t]+/u', ' ', $v) ?? '';
			$v = trim($v, " \t\"");
			// L�mite de longitud defensivo
			if (strlen($v) > $maxLen) {
				$v = substr($v, 0, $maxLen);
			}
			return $v;
		...
		if ($hasHtaccess) {
			...
			if (!$this->makeBackup($backupTarget)) {
				...
				// $this->logger->warning('Backup de .htaccess fall�');
			}
		}
		...
		if ($hasHtaccess) {
			...
			// Primer volcado (best-effort); luego haremos escritura at�mica al final
			@File::copy(
		...
		// Limpia l�neas en blanco m�ltiples
		$cleanRules = preg_replace('/\n{3,}/', "\n\n", $cleanRules) ?? $cleanRules;
		$cleanRules = rtrim($cleanRules) . "\n";

		// 3) Construye nuevos bloques seg�n opciones
		$blocks = [];
		...
		// C�digo propio (permitimos l�neas, pero saneadas y sin marcadores)
		...
		if ($sensFiles !== []) {
			// Literalizamos cada �tem para evitar metacaracteres peligrosos.
		...
		// (tambi�n tolera saltos de l�nea accidentales)
		$backendExceptionsRaw = (string) $this->getValue('backend_exceptions');

		// Divide por comas o saltos de l�nea, recorta, y filtra vac�os/duplicados
		...
		foreach ($items as $ex) {
			...
			//   - "qs:algo"   -> excepci�n por query string
			//   - "uri:/ruta" -> excepci�n por URI (patr�n)
			if (stripos($ex, 'qs:') === 0 || stripos($ex, 'uri:') === 0) {
				$norm = $ex; // se sanitiza m�s adelante con $sanitizeHtLine
			} else {
			...
			// L�mite de seguridad (igual que tu l�gica)
		...
		if ($secret !== '') {
			...
			$lines = [
				...
				'RewriteEngine On',
				'',
				'# 0) Excepciones de est�ticos del admin y otros conocidos',
			...
			if (!empty($exceptionsList)) {
				...
				foreach ($exceptionsList as $ex) {
					...
					// Construye patr�n seguro con soporte de '*'
		...
		// 5) Escritura at�mica (tmp ? rename) + permisos
    ...
    function generate_rules()
    {
        ...
        if ($this->getValue("default_banned_list")) {
            ...
            // A�adimos el contenido del fichero por defecto al final del buffer
        ...
        /* Comprobamos si hay que prohibir el acceso a archivos que pueden contener informaci�n sensible o que tengan alguna vulnerabilidad */
        ...
        /* Comprobamos si hay que aplicar c�digo del usuario */
        ...
        /* Comprobamos si hay que establecer protecci�n contra ataques basados en mime*/
        ...
        /* Comprobamos si hay que establecer protecci�n STS (Strict Transport Security) */
        ...
        /* Comprobamos si hay que establecer protecci�n X-Xss-Protection */
    ...
    public function restoreHtaccess(): bool
    {
        ...
        try {
            ...
            foreach ([$srcClean, $dstClean, $tmpClean, $bakClean] as $p) {
                // Si realpath falla (p.ej. archivo no existe a�n), comprobamos prefijo con la ruta normalizada
                $real = realpath($p) ?: $p;
            ...
            // --- Reemplazo at�mico: elimina destino (si existe) y mueve temporal a destino ---
            ...
            if (!File::move($tmpClean, $dstClean)) {
                // Intenta limpiar el temporal si fall� el move
                @File::delete($tmpClean);
            ...
            // Notificaci�n en backend (sin filtrar detalles t�cnicos sensibles al usuario final)
            $app->enqueueMessage(
