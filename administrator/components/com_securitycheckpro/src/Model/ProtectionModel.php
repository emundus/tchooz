...
class ProtectionModel extends BaseDatabaseModel
{
    ...
    public function load($key_name)
    {
        ...
        if(!empty($res)) {
			...
			if ( (is_array($res)) && (array_key_exists($key_name,$res)) )
			{
				return $res[$key_name];
			}
        } 
    ...
    {
        return File::copy(JPATH_SITE . DIRECTORY_SEPARATOR . '.htaccess', JPATH_SITE . DIRECTORY_SEPARATOR . $name);
    }
        ...
        {  
            ...
            } else
            {
				...
				if (version_compare(JVERSION, '4.0', 'ge')) {
					$filename = "default_joomla_htaccess_j4.inc";
				}
                // Leemos el contenido del fichero .htaccess existente y lo guardamos en el buffer.
                $rules .= file_get_contents(JPATH_COMPONENT_ADMINISTRATOR.DIRECTORY_SEPARATOR.'includes'.DIRECTORY_SEPARATOR.$filename);
                $status = File::copy(JPATH_COMPONENT_ADMINISTRATOR.DIRECTORY_SEPARATOR.'includes'.DIRECTORY_SEPARATOR.$filename, JPATH_SITE.DIRECTORY_SEPARATOR.'.htaccess');
            }
        }
    ...
    function restore_htaccess()
    {
        ...
        return File::copy(JPATH_SITE . DIRECTORY_SEPARATOR . '.htaccess.original', JPATH_SITE . DIRECTORY_SEPARATOR . '.htaccess');    
    }
