...
class CpanelModel extends BaseModel
{
    ...
    function __construct()
    {
        ...
        if ( (strstr(strtolower(htmlspecialchars($_SERVER['SERVER_SOFTWARE'])), 'apache')) || (strstr(strtolower(htmlspecialchars($_SERVER['SERVER_SOFTWARE'])), 'litespeed')) || (strstr(strtolower(htmlspecialchars($_SERVER['SERVER_SOFTWARE'])), 'wisepanel')) ) {
            ...
            $server = 'nginx';
        } else if (strstr(strtolower(htmlspecialchars($_SERVER['SERVER_SOFTWARE'])), 'microsoft-iis')) {        
            $server = 'iis';
        }
	...
	/* Función que hace un overwrite de la función execute de Joomla para permitir insertar los triggers en la bbdd */
	function execute($query) {
		...
		if ($conn->query($query) === TRUE) {		  
			...
			Factory::getApplication()->enqueueMessage($conn->error, 'error');
			return false;		 
		}
	}
    ...
    {
		// https://issues.joomla.org/tracker/joomla-cms/38169
		$config = Factory::getConfig();
		$dbprefix = $config->get('dbprefix');
		
		$message = Text::_('COM_SECURITYCHECKPRO_LOCKED_MESSAGE');
        $trigger_name = $component_name . "_" . $option . "_trigger";
        $table_name_joomla_format = $dbprefix . $table_name;
            ...
            if ($table_name == "users") {
                if ($option == "update") {
					if (strstr($this->dbtype,"mysql")) {
						...
						SET @locked=(SELECT storage_value FROM {$dbprefix}securitycheckpro_storage WHERE storage_key = 'locked');
						...
						$query = "CREATE FUNCTION {$trigger_name}() RETURNS trigger AS \${$trigger_name}\$ DECLARE locked integer; BEGIN locked:=(SELECT storage_value FROM {$dbprefix}securitycheckpro_storage WHERE storage_key = 'locked'); IF (locked = 1 and ((OLD.name != NEW.name) OR (OLD.username != NEW.username) OR (OLD.email != NEW.email) OR (OLD.password != NEW.password) OR (OLD.block != NEW.block))) THEN RAISE EXCEPTION '{$message}'; END IF; END; \${$trigger_name}\$ LANGUAGE plpgsql;";
						try{
							$this->execute($query);
						} catch (Exception $e)
						{  							
					...
					if (strstr($this->dbtype,"mysql")) {
						...
						$query = "CREATE FUNCTION {$trigger_name}() RETURNS trigger AS \${$trigger_name}\$ DECLARE locked integer; BEGIN locked:=(SELECT storage_value FROM {$dbprefix}securitycheckpro_storage WHERE storage_key = 'locked'); IF locked = 1 THEN RAISE EXCEPTION '{$message}'; END IF; END; \${$trigger_name}\$ LANGUAGE plpgsql;";
                ...
                if ($option == "update") {
					if (strstr($this->dbtype,"mysql")) {
						...
						$query = "CREATE FUNCTION {$trigger_name}() RETURNS trigger AS \${$trigger_name}\$ DECLARE locked integer; BEGIN locked:=(SELECT storage_value FROM {$dbprefix}securitycheckpro_storage WHERE storage_key = 'locked'); IF (locked = 1 and (OLD.introtext != NEW.introtext OR OLD.fulltext != NEW.fulltext)) THEN RAISE SQLSTATE '02000' USING MESSAGE='{$message}'; END IF; RETURN NULL; END; \${$trigger_name}\$ LANGUAGE plpgsql;";						
					...
					if (strstr($this->dbtype,"mysql")) {
						...
						$query = "CREATE FUNCTION {$trigger_name}() RETURNS trigger AS \${$trigger_name}\$ DECLARE locked integer; BEGIN locked:=(SELECT storage_value FROM {$dbprefix}securitycheckpro_storage WHERE storage_key = 'locked'); IF (locked = 1) THEN RAISE SQLSTATE '02000' USING MESSAGE='{$message}'; END IF; RETURN NULL; END; \${$trigger_name}\$ LANGUAGE plpgsql;";						
                ...
                if ($option == "update") {
					if (strstr($this->dbtype,"mysql")) {
						...
						$query = "CREATE FUNCTION {$trigger_name}() RETURNS trigger AS \${$trigger_name}\$ DECLARE locked integer; BEGIN locked:=(SELECT storage_value FROM {$dbprefix}securitycheckpro_storage WHERE storage_key = 'locked'); IF locked = 1 and ((OLD.extension_id != NEW.extension_id) OR (OLD.name != NEW.name) OR (OLD.element != NEW.element) OR (OLD.folder != NEW.folder) OR (OLD.access != NEW.access) OR (OLD.protected != NEW.protected) OR (OLD.state != NEW.state)) THEN RAISE SQLSTATE '02000' USING MESSAGE='{$message}'; END IF; RETURN NULL; END; \${$trigger_name}\$ LANGUAGE plpgsql;";						
    ...
    function update_extra_query_update_sites_table($site_id,$dlid)
    {
		...
		if ( !is_int($site_id) ) {
			Factory::getApplication()->enqueueMessage(Text::_('COM_SECURITYCHECKPRO_SITE_ID_NOT_VALID'), 'error');	
			return;
		}
		...
		Factory::getApplication()->enqueueMessage(Text::_('COM_SECURITYCHECKPRO_DOWNLOAD_ID_UPDATED'));
	}
