...
class FirewallconfigModel extends BaseModel
{
    ...
    function filter_data($array_data,&$pagination)
    {
		...
		$search = "";
		$search_filter = $this->state->get('filter.lists_search');
        ...
        $pagination = new Pagination($this->total, $upper_limit, $lower_limit);
    ...
    function import_list()
    {
        ...
        if ( empty($lista) ) {
            Factory::getApplication()->enqueueMessage(Text::_("No list defined"), 'warning');
            return false;
        }
        ...
        if ($upload_res) {
            ...
            foreach($ip_to_validate as $ip) {
                ...
                if (!$ip_valid) {                
                    Factory::getApplication()->enqueueMessage(Text::_("COM_SECURITYCHECKPRO_INVALID_FILE_FORMAT") . $ip_valid, 'warning');
                    return false;
                } else {
					$object = (object)array(
						'ip'        => $ip_valid
    ...
    function send_email_test()
    {
        ...
        try {
            ...
            Factory::getApplication()->enqueueMessage($e, 'notice');
            $send = false;
        }
