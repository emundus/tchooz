...
class FirewallconfigModel extends BaseModel
{
	...
	/**
	 * Limitbox gen�rico para una pesta�a (blacklist|dynamic_blacklist|whitelist)
	...
	/**
	 * Listado + paginaci�n con filtro global "filter.lists_search"
	...
	public function getListWithPagination(string $contextKey, string $tableName, array $searchColumns = ['ip']): array
	{
		...
		// ---- START: primero {$prefix}limitstart (propio de la paginaci�n con prefix)
    ...
    /**
     * Funci�n que elimina IPs de la lista negra din�mica
     *
     *
     * @return  void
     *     
    ...
    function deleteip_dynamic_blacklist()
    {
        ...
        // Obtenemos los valores de las IPs que ser�n eliminados de la lista negra din�mica
   ...
   /**
     * Funci�n que chequea si la opci�n de control center est� habilitada en el firewall
     *
     *
     * @return  bool
     *     
    ...
    /**
     * Funci�n que a�ade una ip al fichero que ser� consumido por el control center si el plugin 'Connect' est� habilitado
     *
    ...
    /**
     * Funci�n para a�adir/borrar una ip a una lista
     *
    ...
    function manage_list($type,$action,$ip=null,$check_own=true,$remote=false)
    {
        ...
        case "add":
            ...
            //IPv4
            if (strstr($ip_to_add, '*')) { // Si existe alg�n comod�n, lo reemplazamos por el d�gito '0'
            ...
            // Si la IP es la del cliente no la a�adimos para no bloquearnos, excepto cuando la petici�n provenga del url inspector
            if ($check_own) {
   	...
   	/**
     * Funci�n que sube un fichero de IPs de la extensi�n Securitycheck Pro (previamente exportado) y lo a�ade a la bbdd
     *
     * @return  bool
     *     
     */
    public function import_list(): bool
    {
        ...
        try {
            ...
            // Transacci�n + inserciones por lotes
            $db->transactionStart();
            ...
            for ($i = 0, $n = count($validIps); $i < $n; $i += $chunkSize) {
                ...
                // Intento de inserci�n por lote (port�til). Si hay conflictos por PK, degradamos a per-row.
                $query = $db->getQuery(true)
                ...
                try {
                    ...
                    // Degradar a inserci�n individual; los duplicados se omiten (PK)
                    foreach ($chunk as $ip) {
                        $q = $db->getQuery(true)
                            ->insert($db->quoteName($table))
                            ->columns([$db->quoteName('ip')])
	...
	/**
     * Funci�n que manda un email de prueba utilizando los par�metros establecidos
     *
     *
     * @return  void
     *     
    ...
    function send_email_test()
    {
        ...
        // A�adimos un mensaje de que todo ha funcionado correctamente
        if ($send === true) {
    ...
    /**
     * Funci�n que chequea si el plugin pasado como argumento est� instalado
     *
