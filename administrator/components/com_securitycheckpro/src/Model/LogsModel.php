...
class LogsModel extends ListModel
{
    ...
    protected function populateState($ordering = 'a.time', $direction = 'DESC')
    {
		...
		// Defaults: m�s recientes primero
		...
		if ($dirFromRequest === '') {
			...
			// Asegura tambi�n la columna por defecto si no vino en la request
			$orderFromRequest = $input->getCmd('filter_order', '');
			if ($orderFromRequest === '') {
				$this->setState('list.ordering', 'a.time');
			}
			// Opcional: resetea la paginaci�n al cambiar orden por defecto
			$this->setState('list.start', 0);
    ...
    protected function loadFormData(): array
    {
        ...
        // S�lo lo inyectamos si el filtro a�n no tiene valor
	...
	/**
     * Chequea si un string es una fecha v�lida
     *
    ...
    /**
     * Cambia el estado de marcado (le�do/no le�do) de los logs indicados.
     *
     * - Si $uids es null, leer� los IDs de la request: input['cid'] (array).
     * - Sanitiza y normaliza los IDs, evitando valores no num�ricos y duplicados.
     * - Usa UPDATE con WHERE IN y par�metros ligados (sin concatenaciones).
     *
     * @param  bool                $setRead true => marcado=1 (le�do), false => marcado=0 (no le�do)
    ...
    public function markLogs(bool $setRead, ?array $uids = null): void
    {
        ...
        // Convierte in-place y vuelve a mapear a int por si el helper no actu� por referencia
        ...
        $marked = $setRead ? 1 : 0; // variable (no expresi�n)
    ...
    {
        // En whitelist no aplicamos la restricci�n de �propia IP�
        $this->addIpsToList('whitelist', false);
    }

    /**
     * N�cleo com�n para a�adir IPs a blacklist/whitelist desde los logs seleccionados.
     *
     * @param 'blacklist'|'whitelist' $listType
     * @param bool                    $preventSelfBlock  Evita a�adir la IP del cliente (s�lo blacklist)
     */
    private function addIpsToList(string $listType, bool $preventSelfBlock): void
    {
        ...
        // 3) IPs candidatas (normalizadas, �nicas)
        ...
        // 4) Evita bloquear IP propia si aplica (s�lo blacklist)
        $selfIpRemoved = false;
        if ($preventSelfBlock) {
            try {
                ...
                // si falla la detecci�n de IP cliente, no impedimos el flujo
            }
        }
        ...
        // 8) Marca los logs como le�dos usando tu nuevo m�todo
        $this->markLogs(true, $ids);
    }
	...
	/**
	 * Vac�a toda la tabla de logs (backend).
	 * Requiere permiso fuerte espec�fico.
	 */
	...
	/**
	 * A�ade de forma segura un elemento a una lista CSV.
	 *
	 * Reglas:
	 *  - No guarda HTML escapado (escapa s�lo al mostrar).
	 *  - Normaliza espacios y elimina controles.
	 *  - Rechaza elementos que contengan comas.
	 *  - Evita duplicados (comparaci�n case-insensitive).
	 *
	 * @param string $csv         Lista actual separada por comas (p.ej. "A,B,C")
	 * @param string $newElement  Elemento a a�adir (sin coma)
	 *
	...
	{
		// Limpieza b�sica del nuevo elemento
		...
		if ($cleanNew === '') {
			// No a�adimos vac�os; devolvemos tal cual normalizado
	...
	function parseCsv(string $csv): array
	{
		...
		foreach ($raw as $val) {
			...
			if ($v === '' || str_contains($v, ',')) {
				// Descarta vac�os y entradas corruptas con coma
				continue;
			}
	...
	/**
	 * Normaliza un CSV (�til para entradas existentes).
	 *
	 * @param string $csv
	 * @return string
	 */
	...
	/**
     * A�ade excepciones a la configuraci�n a partir de logs seleccionados.
     *
     * Seguridad:
     *  - Tipado estricto y retorno void
     *  - Consultas preparadas con par�metros tipados
     *  - Allow-list de tags -> clave de par�metro
     *  - Coincidencia exacta por elemento (sin 'stristr')
     *  - Saneado/validaci�n del valor 'component'
     *
     * @return void
     */
    public function add_exception(): void
    {
        ...
        // --- Carga de par�metros del firewall (storage_key = 'pro_plugin') ---
        $query = $db->getQuery(true)
            ->select($db->quoteName('storage_value'))
            ->from($db->quoteName('#__securitycheckpro_storage'))
        ...
        if (is_string($rawParams) && $rawParams !== '') {
            try {
                ...
                // Si hay JSON corrupto, contin�a con params vac�os para no romper UX
                $params = [];
            }
        }

        // Allow-list: tag_description => clave de configuraci�n
        $tagToParam = [
        ...
        foreach ($uids as $uid) {
            ...
            // Asegura que el �ndice existe como CSV (compatibilidad)
            $currentCsv = (string) ($params[$paramKey] ?? '');

            // A�ade si no existe (comparaci�n exacta tras normalizar)
        ...
        // --- Guarda configuraci�n s�lo si hubo cambios ---
        if ($somethingAdded) {
            try {
     ...
     *
     * @return non-empty-string|string '' si inv�lido
     */
    private function sanitizeComponent(string $value): string
    {
        ...
        // Permitimos letras, d�gitos, guion, guion bajo, punto, dos puntos y barra
    ...
    /**
     * Dada una lista CSV, a�ade un elemento si no existe (comparaci�n exacta tras trim).
     *
     * @param non-empty-string $value
     * @return array{0:string,1:bool} [csvNormalizado, a�adido]
     */
    private function addToCsvListUnique(string $csv, string $value): array
    {
        // Explota en items, normaliza espacios y filtra vac�os
        $items = array_values(
            array_filter(
                array_map(
        ...
        // Coincidencia exacta (case-sensitive). Si prefieres insensible a may�sculas, usa strtolower en ambos lados.
