...
class SecuritycheckproModel extends ListModel
{
    ...
    /**
     * Carga estado (filtros, orden, paginaci�n) desde la request.
     */
     ...
     * (sin filtrar por los selects de la tabla), para las métricas del dashboard.
     *
    ...
    function chequear_vulnerabilidades()
    {
		...
		// Versi�n de Joomla instalada
		$local_joomla_branch = explode('.', JVERSION)[0];
		
        // Extraemos los componentes vulnerables para nuestra versi�n de Joomla
        $query = $db->getQuery(true)
            ->select('*')
		...
		foreach ($vuln_components as $vulnerable_product)
        {
			...
			if ($components_key === false) {
				// El producto vulnerable no est� instalado
			} else {
				if ( $components[$components_key]['sc_type'] == $vulnerable_product['vuln_type'] ) {
					$modvulnversion = $vulnerable_product['modvulnversion']; //Modificador sobre la versi�n de la extensi�n
                    $db_version = $components[$components_key]['Installedversion']; // Versi�n de la extensi�n instalada
                    $vuln_version = $vulnerable_product['Vulnerableversion']; // Versi�n de la extensi�n vulnerable
					...
					if ( ($version_compare) || ($vuln_version == '---') ) {
                        ...
                        try
                        {
							// A�adimos los datos a la tabla 'securitycheck_vuln_components' si no existen ya   
							if (is_null($exists)) {
	 ...
	 *  - Par�metros enlazados (bind) en vez de concatenar
	 *  - Normalizaci�n de strings y l�mites de longitud
	 *  - Manejo de errores sin filtrar mensajes de BBDD al usuario
	 *
	 * @param string      $nombre      Valor para la condici�n WHERE (del campo $campo)
	 * @param string      $database    Nombre l�gico de la tabla (sin prefijo), debe estar en la allow-list
	 * @param string      $campo       Campo para la condici�n WHERE (p.ej. 'Product'), must be allowed
	 ...
	 *
	 * @return bool True si se actualiz� al menos una fila, False en caso contrario o error
	 */
	function actualizarRegistro(
		...
		// ---- Allow-lists ----
		// Mapea nombres l�gicos a tablas reales (sin prefijo)
		$allowedTables = [
		...
		// Normalizaci�n/defensas b�sicas de datos (no afecta al bind, solo higiene)
		...
		try {
			...
			// Devolvemos true solo si afect� al menos a una fila
	...
	function buscar_registro(string $nombre, string $database, string $campo): bool
	{
		...
		// --- Allow-list de tabla y campo (evita inyecci�n en identificadores) ---
		...
		if (!in_array($campo, $allowedFields, true)) {
			// Por seguridad, si el campo no es v�lido, devolvemos false
			return false;
		}
		...
		try {
			$db->setQuery($q);
			// Si existe alguna fila, loadResult() devolver� '1'
	...
	function insertar_registro(string $nombre, string $version, string $tipo): bool
	{
		...
		// Normalizaci�n m�nima (no hace falta escapar: insertObject crea los bindings)
		$nombre  = trim($nombre);
	 ...
	 * Mantiene 'Joomla!' como excepci�n.
	 *
	...
	function eliminar_componentes_desinstalados(): void
	{
		...
		// DELETE con NOT EXISTS (port�til y seguro)
		$q = $db->getQuery(true)
		...
		try {
			...
			if ($deleted > 0) {
				// Compatibilidad con tu l�gica original: dejamos el dato en input
				$input->set('comp_eliminados', Text::_('COM_SECURITYCHECKPRO_DELETED_COMPONENTS') . $deleted);
				// Y adem�s mostramos un mensaje informativo en backend
	...
	function actualizarbbdd(array $registros): void
	{
		...
		if (!$needSync) {
			try {
				...
				// Si no podemos leer, forzamos sincronizaci�n inicial
				$needSync = true;
			}
		}
		...
		foreach ($registros as $row) {
			...
			$product = mb_substr($product, 0, 190);    // �ndices/�nicos suelen estar <191 chars en utf8mb4
		...
		// --- 3) Transacci�n: TRUNCATE + inserciones ---
		try {
			$db->transactionStart();

			// TRUNCATE espec�fico por driver
			...
			// JSON malformado en alg�n manifest_cache: no aborta toda la sync; puedes loguear y continuar.
    ...
    /**
	 * Obtiene la fecha de actualizaci�n del �ltimo componente a�adido
	 * a la tabla #__securitycheckpro_db por el plugin 'Update Database'.
	 *
	 * @return string Fecha (formato de la columna `published`) o cadena vac�a si no hay registros o en caso de error
	 */
	...
	public function filter_vulnerable_extension(string $product): string
	{
		...
		try {
			...
			$msg = Text::sprintf('JERROR_LOADING_MENUS', $e($ex->getMessage())); // reutilizamos texto gen�rico
