...
class OnlinechecksModel extends ListModel
{
    ...
    /**
     * Carga estado de filtros/paginaci�n/orden.
     *
    ...
    protected function populateState($ordering = null, $direction = null): void
    {
        ...
        // Filtro de b�squeda (persiste entre peticiones)
        ...
        // L�mite por p�gina (usa list_limit global como valor por defecto)
    ...
    /**
     * Construye la consulta base que `ListModel` paginar� autom�ticamente.
     */
    ...
    /**
     * Recuento global de envíos online, archivos comprobados y amenazas encontradas
     * (sin filtrar por búsqueda), para las métricas del dashboard.
     *
   ...
   /**
	...
	{
		// Requiere POST + token CSRF v�lido
		...
		// Carpeta base donde residen los scans (ruta can�nica)
		...
		// Normaliza: quita vac�os, duplica, castea y limita tama�o razonable
		...
		foreach ($filenames as $name) {
			...
			//    Permitimos letras, n�meros, punto, guion y guion bajo.
			...
			if (!is_file($resolved)) {
				// Si no existe, intentamos limpiar igualmente el registro para no dejar hu�rfanos
				try {
					$query = $db->getQuery(true)
					...
					// silencio: no elevamos, s�lo informamos
				}
			...
			// 4) Si el fichero se borr�, elimina su registro asociado en BD
			try {
				$query = $db->getQuery(true)
					->delete($db->quoteName('#__securitycheckpro_online_checks'))
					->where($db->quoteName('filename') . ' = ' . $db->quote($name));
				...
				// El fichero ya est� borrado; reportamos el error de BD pero no revertimos
	...
	/**
	 * Extrae filas de '#__securitycheckpro_online_checks' con paginaci�n/filtrado seguros.
	 *
	...
	public function load(?string $key_name = null): array
	{
		...
		// Filtrado por b�squeda en memoria (si la tabla crece convendr�a moverlo a SQL)
		...
		// Paginaci�n est�ndar Joomla (usa list.start / list.limit)
		...
		// Si limit == 0 -> sin l�mite
		if ($limit > 0) {
			$rows = array_slice($rows, $start, $limit);
		} elseif ($start > 0) {
			// Si no hay l�mite pero s� desplazamiento, aplicamos desde start al final
    ...
    function download_log_file()
    {
		...
		// 1) Selecci�n (array) desde el formulario
		...
		//    - desactiva compresi�n
		if (ini_get('zlib.output_compression')) { @ini_set('zlib.output_compression', 'Off'); }
		//    - cierra la sesi�n para liberar el handler
		if (session_status() === PHP_SESSION_ACTIVE) { @session_write_close(); }
		//    - elimina cabeceras que pudo a�adir Joomla/plugins
		if (function_exists('header_remove')) { @header_remove(); }

		// 5) Cabeceras de descarga (s�lo header() nativo)
		...
		// Asegura c�digo de estado correcto antes de enviar cabeceras
		...
		while (!feof($fp)) {
			...
			if ($buf === false) { break; }
			echo $buf;
			// Env�a al cliente de inmediato
			if (function_exists('fastcgi_finish_request')) {
				// No llamar aqu�; lo usamos al final. Mantenemos flush cl�sico:
			}
			flush();
		}
		fclose($fp);

		// En FPM podr�as terminar la respuesta aqu�:
		if (function_exists('fastcgi_finish_request')) {
