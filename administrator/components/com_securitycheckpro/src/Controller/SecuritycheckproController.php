...
class SecuritycheckproController extends SecuritycheckproBaseController
{
	...
	*/
     ...
     */
    function cancel() {
        $msg = Text::_('Operaci�n cancelada');
    ...
    function csv_export(): void
	{
		...
		if ($out === false) {
			// Si fallara (muy raro), volvemos al flujo de la p�gina
		...
		foreach ($rows as $row) {
			...
			// Escribir la fila; fputcsv se encarga de escapado (comillas, comas, saltos de l�nea)
			fputcsv($out, $row);
		}

		fclose($out);
		// Terminamos aqu� para que Joomla no a�ada HTML despu�s del CSV
		$app->close();
	}    
	...
	/**
     * A�adir componente como excepcion
	 * @return  void
     */
