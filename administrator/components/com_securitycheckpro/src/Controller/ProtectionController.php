 ...
 */
class ProtectionController extends SecuritycheckproBaseController
{
    ...
    function protect()
    {
        ...
        if ($status) {
            $this->setRedirect($url, Text::_('COM_SECURITYCHECKPRO_PROTECTION_APPLIED'));
        } else 
        {
            $this->setRedirect($url, Text::_('COM_SECURITYCHECKPRO_PROTECTION_NOTAPPLIED'), 'error');
        }
    ...
    function delete_htaccess()
    {
        ...
        $url = 'index.php?option=com_securitycheckpro&controller=protection&view=protection&'. Session::getFormToken() .'=1';
        if ($status) {
            $this->setRedirect($url, Text::_('COM_SECURITYCHECKPRO_HTACCESS_DELETED'));
        } else
        {
            $this->setRedirect($url, Text::_('COM_SECURITYCHECKPRO_HTACCESS_NOT_DELETED'), 'error');
        }
    ...
    function restore_htaccess()
    {
        ...
        {
            $this->setRedirect($url, Text::_('COM_SECURITYCHECKPRO_ORIGINAL_HTACCESS_NOT_RESTORED'), 'error');
        }
    ...
    function save_default_user_agent()
    {
        ...
        $status = File::write(JPATH_COMPONENT_ADMINISTRATOR.DIRECTORY_SEPARATOR.'includes'.DIRECTORY_SEPARATOR.'user_agent_blacklist.inc', $new_user_agents_blacklist);
        $url = 'index.php?option=com_securitycheckpro&controller=protection&view=protection&'. Session::getFormToken() .'=1';
        if ($status) {
            $this->setRedirect($url, Text::_('COM_SECURITYCHECKPRO_CHANGES_SAVED'));
        } else 
        {
            $this->setRedirect($url, Text::_('COM_SECURITYCHECKPRO_CHANGES_NOT_SAVED'), 'error');
        }
