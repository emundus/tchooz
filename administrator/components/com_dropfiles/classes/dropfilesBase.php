...
class DropfilesBase
{
    ...
    public static function initComponent()
    {
        ...
        if (self::isJoomla30()) {
            ...
            $document->addScript($uri_dropfiles_assets . '/js/jquery-ui-1.13.0.min.js');
            $document->addStyleSheet($uri_dropfiles_assets . '/css/ui-lightness/jquery-ui-1.13.0.min.css');
        ...
        if (class_exists('DropfilesCloudConnector')) {
            $cloudConnector = new DropfilesCloudConnector();
            $cloudConnector->init();
        }
        ...
        if ($view === 'upload') {
            $document->addScript($uri_dropfiles_assets . '/js/upload.js');
        } else {
...
//        }
    ...
    /**
     * Get Auth upload files on frontend
     *
     * @return mixed
     */
    public static function getAuthUploadFilesOnFrontend()
    {
        return JFactory::getUser()->authorise('com_dropfiles.uploadfilesfrontend', 'com_dropfiles');
    }
    ...
    public static function onShowFrontFile($options)
    {
        ...
        $gaDownloadTracking = (int) $options['componentParams']->get('ga_download_tracking', 0);
        $doc = JFactory::getDocument();
        JHtml::_('jquery.framework');
        $doc->addStyleSheet(JURI::base('true') . '/plugins/dropfilesthemes/default/style_ver5.4.css');
        $doc->addScriptDeclaration('ga_download_tracking="' . $gaDownloadTracking . '";');
        $doc->addScript(JURI::base('true') . '/components/com_dropfiles/assets/js/colorbox.init.js');
        $doc->addScript(JURI::base('true') . '/components/com_dropfiles/assets/js/gatracking.js');
        $content = '';
