    ...
    public function display($tpl = null)
    {
        ...
        if ($app->input->get('layout') === 'form') {
            ...
            if (!empty($lstAllFile)) {
                ...
                $ordering = JFactory::getApplication()->input->getCmd('orderCol', $params->ordering);
                $direction = JFactory::getApplication()->input->getCmd('orderDir', $params->orderingdir);
                $files = DropfilesHelper::orderingMultiCategoryFiles($files, $ordering, $direction, $category_id);
            }
            ...
            foreach ($files as $file) {
                ...
                $pendingClass = '';
                $pendingMessage = '';
                if (!empty($pendingList) && array_key_exists($file->id, $pendingList)) {
                    $pendingClass = 'is_pending';
                    $pendingMessage = '<button class="dropfiles-pending-btn">' . JText::_('COM_DROPFILES_FILES_PENDING_MESSAGES') . '</button>';
                }
                $content .= '<tr class="file ' . $remote_file . ' ' . $unpublish . ' ' . $pendingClass . '" data-id-file="';
