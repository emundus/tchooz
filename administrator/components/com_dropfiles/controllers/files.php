...
class DropfilesControllerFiles extends JControllerForm
{
    ...
    public function upload()
    {
        ...
        $generatePreview = new DropfilesModelGeneratepreview();
                    ...
                    if ($joinFiles === false) {
                        ...
                        if ($category->type === 'googledrive') {
                            ...
                            if ($result && (int) $result['file']['error'] === 0) {
                                ...
                                if ($isPending) {
                                    $file_data['state'] = '0';
                                    $pendingList[$result['file']['id']] = true;
                                    DropfilesComponentHelper::setParams(array('_dropfiles_pending_upload_files' => $pendingList));
                                } else {
                                    $file_data['state'] = '1';
                                }
    ...
    public function movefile()
    {
        ...
        if (!$this->allowEdit() && !( $this->allowEditOwn($id_category) && $this->allowEditOwn($active_category_id))) {
        ...
        if ($active_category->type === 'default' && $target_category->type === 'default') {
            ...
            $onedrive = new DropfilesOneDrive();
            $cloudId = DropfilesCloudHelper::getOneDriveIdByTermId($id_category);
            $result = $onedrive->moveFileWithInfo($id_file, $cloudId);
            ...
            if ($result) {
                ...
                if ($result) {
                    ...
                    $file_data['file_id']       = DropfilesCloudHelper::replaceIdOneDrive($result['id']);
                    $file_data['ext']           = strtolower(JFile::getExt($result['title']));
                    $file_data['size']          = $result['size'];
                    $file_data['catid']         = $target_category->cloud_id;
                    $file_data['path']          = '';
                    $file_data['created_time']  = date('Y-m-d H:i:s', strtotime($result['created_time']));
                    $file_data['modified_time'] = date('Y-m-d H:i:s', strtotime($result['modified_time']));
                    $file_data['author']        = $user->get('id');
                    $modelOnedrive->save($file_data);
                }
            }
    ...
    public function copyfile()
    {
        ...
        if ($active_category->type === 'default' && $target_category->type === 'default') {
            ...
            if ($result) {
                ...
                $file_data['catid']                 = $target_category->cloud_id;
                $file_data['path']                  = '';
                $file_data['created_time']          = date('Y-m-d H:i:s', strtotime($file->created_time));
                $file_data['modified_time']         = date('Y-m-d H:i:s', strtotime($file->modified_time));
                $file_data['author']                = $user->get('id');
                $file_data['file_tags']             = $result['file']['file_tags'];
                $modelOnedrive->save($file_data);
            }
        ...
        if ($category) {
            switch ($category->type) {
                ...
                default:
                    ...
                    $generatePreview = new DropfilesModelGeneratepreview();
                    $generatePreview->removeFileFromQueue($id_file);
            }
            // Update files count
            $categoriesModel = $this->getModel('Categories', 'DropfilesModel');
            $categoriesModel->updateFilesCount();
     ...
     * @since  version
     */
    private function canUpload($id_category)
    {
        $model = $this->getModel('category');
        $category = $model->getItem($id_category);
        $canDo = DropfilesHelper::getActions();
        ...
        if ($canDo->get('core.edit')) {
            ...
            if ($canDo->get('core.edit.own')
                ...
                if ($canDo->get('com_dropfiles.uploadfilesfrontend')) {
                    $canUpload = true;
                }
            }
        }
