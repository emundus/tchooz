...
class Com_DropfilesInstallerScript
{
    ...
    public static function install()
    {
       ...
       // $dbo->execute();
        ...
        $query = 'CREATE TABLE IF NOT EXISTS `#__dropfiles_options` (
                        `id` int(11) NOT NULL AUTO_INCREMENT,
    ...
    public function uninstall()
    {
        ...
        $queries[] = 'DROP TABLE IF EXISTS `#__dropfiles_options`';
    ...
    public function update()
    {
        ...
        if (version_compare($this->oldRelease, '6.1.0', 'lt')) {
            $query = 'CREATE TABLE IF NOT EXISTS `#__dropfiles_options` (
                          `id` int(11) NOT NULL AUTO_INCREMENT,
                          `name` varchar(255) NOT NULL,
                          `value` longtext NOT NULL,
    ...
    public function postflight($type, $parent)
    {
        ...
        if ($type === 'install' || $type === 'update') {
            ...
            foreach ($extensions->children() as $extension) {
                ...
                // Abort unused extension
                $folderName = $folder;
                if (is_object($folder)) {
                    $folderArr = (array) $folder;
                    $folderName = $folderArr['0'];
                }

                if (DropfilesBase::isJoomla40() && $folderName === 'dropfiles-search') {
                    continue;
                }

                if (DropfilesBase::isJoomla30() && $folderName === 'dropfiles-finder') {
                    continue;
                }
