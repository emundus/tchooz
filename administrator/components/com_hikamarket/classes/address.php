...
class hikamarketAddressClass extends hikamarketClass {
	...
	public function frontSaveForm($user_id, $area = '') {
		...
		if(!empty($data[$currentTask])) {
			...
			if(!empty($data[$currentTask]['address_id']) && is_numeric($data[$currentTask]['address_id']))
				$user_address->address_id = (int)$data[$currentTask]['address_id'];
	...
	public function save(&$addressData, $type = null) {
		...
		if(!empty($addressData->address_id)) {
			...
			if(isset($addressData->address_user_id) && (int)$oldData->address_user_id != (int)$addressData->address_user_id) {
				return false;
			}
