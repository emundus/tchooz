...
class hikamarketDiscountClass extends hikamarketClass {
	...
	public function frontSaveForm($task = '', $acl = true) {
		...
		if(!hikamarket::acl('discount/edit/shippingpercent')) { unset($discount->discount_shipping_percent); }
...
function hikamarket_discount_changeVendor(el) {
	...
	public function beforeCouponCheck(&$coupon, &$total, &$zones, &$products, &$display_error, &$error_message, &$do) {
		...
		foreach($products as &$product) {
			if($product->product_parent_id == 0 || $product->product_vendor_id != 0)
				continue;
			if(empty($variant_products[(int)$product->product_parent_id]))
				$variant_products[(int)$product->product_parent_id] = array();
			$variant_products[(int)$product->product_parent_id][] = &$product;
		}
		unset($product);
		...
		foreach($products as &$product) {
			if((int)$product->product_vendor_id == (int)$coupon->discount_target_vendor || ((int)$product->product_vendor_id <= 1 && (int)$coupon->discount_target_vendor == -1)) {
				...
				if(isset($variant_products[(int)$product->product_id])) {
					foreach($variant_products[(int)$product->product_id] as &$variant_product) {
						$vendor_products[] = &$variant_product;
					}
					unset($variant_product);
					unset($variant_products[(int)$product->product_id]);
				}
			}
		}
		unset($product);
		unset($variant_products);
