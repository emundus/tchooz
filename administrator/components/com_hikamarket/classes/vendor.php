...
class hikamarketVendorClass extends hikamarketClass {
	...
	public function save(&$vendor) {
		...
		$context = 'com_hikamarket.vendor';
		$app->triggerEvent('onFinderBeforeSave', array($context, $vendor, $new));
		...
		$app->triggerEvent('onFinderAfterSave', array($context, $vendor, $new));
	...
	public function addAlias(&$vendor, $force_save = true) {
		if(empty($vendor->vendor_alias)) {
			$vendor->alias = !empty($vendor->vendor_name) ? $vendor->vendor_name : @$vendor->old->vendor_name;
		} else {
			$vendor->alias = $vendor->vendor_alias;
		}
		if(empty($vendor->alias))
			return;
		...
		if(!$jconfig->get('unicodeslugs')) {
			$lang = JFactory::getLanguage();
			$vendor->alias = $lang->transliterate($vendor->alias);
		}
		if(method_exists($app,'stringURLSafe'))
			$vendor->alias = $app->stringURLSafe($vendor->alias);
		else
			$vendor->alias = JFilterOutput::stringURLSafe($vendor->alias);

		$shopConfig = hikamarket::config(false);
		if($shopConfig->get('sef_remove_id', 0)) {
			$int_at_the_beginning = (int)$vendor->alias;
			if($int_at_the_beginning)
				$vendor->alias = $shopConfig->get('alias_prefix', 'v') . $vendor->alias;
		}

		if($force_save && (empty($vendor->vendor_alias) || $vendor->vendor_alias != $vendor->alias)) {
			$vendor_save = new stdClass();
			$vendor_save->vendor_id = $vendor->vendor_id;
			$vendor_save->vendor_alias = $vendor->alias;
			parent::save($vendor);			
		}
	}
