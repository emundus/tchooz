	...
	public function calculateVendorPrice($vendor_id, &$v_order, &$products, $coupon) {
		...
		foreach($order_products as &$product) {
			...
			if(!empty($coupon) && !empty($coupon->products) && empty($coupon->all_products)) {
				$discount_flat_amount = $coupon->discount_flat_amount;
				if(!$vendor_price_with_tax && isset($coupon->discount_flat_amount_without_tax))
					$discount_flat_amount = $coupon->discount_flat_amount_without_tax;
				foreach($coupon->products as $couponProduct) {
					...
					if(isset($couponProduct->processed_discount_value)) {
					...
					} elseif(isset($coupon->discount_percent_amount_orig) && bccomp($coupon->discount_percent_amount_orig, 0, 5) !== 0) {
						$full_price *= floatval((100 - floatval($coupon->discount_percent_amount_orig)) / 100);
						...
						$full_price -= floatval($discount_flat_amount) * $percent;
					}
				}
			}
		...
		if(!empty($coupon) && !empty($coupon->discount_target_vendor) && (int)$coupon->discount_target_vendor > 0 && (empty($coupon->products) || !empty($coupon->all_products))) {
			...
			if(bccomp($discount_flat_amount, 0, 5) !== 0 && (!isset($coupon->discount_percent_amount_orig) || bccomp($coupon->discount_percent_amount_orig, 0, 5) === 0)) {
				$coupon_percentage = (float)($coupon->vendors[$vendor_id] / $coupon->products_full_price);

				$v_order->order_discount_price = floatval($discount_flat_amount) * $coupon_percentage;
				...
				$ret -= floatval($discount_flat_amount) * $coupon_percentage;
			...
			foreach($fees as $fee) {
				...
				if(
				 ...
				 ($global_mode && (($total_quantity >= $fee->fee_min_quantity || $fee->fee_min_quantity <= 1) && ($total_price >= $fee->fee_min_price || $fee->fee_min_price == 0)))
					...
					if(!$global_mode && $product_vendor_unit_price < 0 && $fee->fee_percent >= 0) {
						$product_full_price = 0;
						$product_vendor_unit_price = 0;
					}
