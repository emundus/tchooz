...
'(1,0,1,'.$e.','.$e.',0,'.$e.','.$this->db->Quote('*').','.$e.','.$e.')';
	...
	public function displayInfo() {
		...
		$background = version_compare(JVERSION,'5.0','<') ? 'background:#f6f6f6;' : '';
		echo '
<div style="'.$background.';border:2px solid #aabb33;max-width:100%;text-align:left;border-radius:6px;padding:20px 30px;margin:5px 0px 10px;">
	<h2><img src="'.HIKAMARKET_IMAGES.'icon-48/hikamarket.png" alt=""/> '.HIKAMARKET_NAME.' 5.0.0 <small>'.JText::_('HIKAM_INSTALL_FOR').' HikaShop</small></h2>
...
class com_hikamarketInstallerScript {
	...
	public function preflight($type, $parent) {
		...
		if(version_compare($hikashopConfig->get('version', '1.0'), '5.0.0', '<')) {
			echo '<h1>This extension works with HikaShop 5.0.0 or newer.</h1>'.
				'<h2>Please install the latest version of HikaShop before.</h2>'.
				'installation abort.';

			$app = JFactory::getApplication();
			if($type == 'update')
				$app->enqueueMessage('Cannot update HikaMarket 5.0.0 without HikaShop 5.0.0 or newer', 'warning');
			else
				$app->enqueueMessage('Cannot install HikaMarket 5.0.0 without HikaShop 5.0.0 or newer', 'warning');

			$joomConf = JFactory::getConfig();
			$debug = $joomConf->get('debug');
			if($debug) {
				$app->enqueueMessage('Cannot install HikaMarket 5.0.0 without HikaShop 5.0.0 or newer', 'error');
			}
			return false;
		}
