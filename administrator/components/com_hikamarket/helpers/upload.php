...
class hikamarketUploadHelper {
	...
	protected function validate($uploaded_file, $file, $error, $options, $index) {
		if($error) {
			$file->error = $error;
			return false;
		}
		...
		if($options['type'] == 'file') {
			...
			if((!empty($this->options['max_width']) && $img_width > $this->options['max_width']) || (!empty($this->options['max_height']) && $img_height > $this->options['max_height'])) {
				$file->error = 'maxResolution';
				return false;
			}
			if((!empty($this->options['min_width']) && $img_width < $this->options['min_width']) || (!empty($this->options['min_height']) && $img_height < $this->options['min_height'])) {
