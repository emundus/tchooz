...
class hikamarket {
	...
	public static function footer() {
		...
		if(hikaInput::get()->getInt('marketbuild',0)) $text .= '<!-- 2409092018 -->';
	...
	public static function getAclUser($user = null, $vendor = null) {
		if($user === null)
		...
		$db = JFactory::getDBO();
		if($vendor !== null && isset($vendor->vendor_id)) {
			$vendor_id = (int)$vendor->vendor_id;
		} else
			$vendor_id = self::loadVendor(false, false);
		$query = 'SELECT user_id, user_access FROM '.hikamarket::table('vendor_user').' WHERE user_id = '.(int)$user->user_id.' AND vendor_id = '.(int)$vendor_id;
		$db->setQuery($query);
		$user_vendor = $db->loadObject();
		if(!empty($user_vendor)) {
			if($user_vendor->user_access == 'all')
				$user_vendor->user_access = '*';
			if(!empty($user_vendor->user_access) && strpos($user_vendor->user_access, '/') === false) // Compat
				$user_vendor->user_access = str_replace('_', '/', $user_vendor->user_access);
			$user_access = explode(',', trim(strtolower($user_vendor->user_access), ','));
			sort($user_access, SORT_STRING);
		}
		unset($user_vendor);

		if(empty($user_access) && !empty($user->user_vendor_access)) {
...
} else if(HIKASHOP_J50) {
	$dark_mode = (int)$configMarket->get('dark_mode', '2');
	$path = HIKAMARKET_CSS.'backend_dark.css';
	if($dark_mode==2) {
		...
		$dark_mode = $userColorScheme === 'light' ? 0 : ($userColorScheme === 'dark' ? 1 : $dark_mode);
	}
	if($dark_mode === 1) {
