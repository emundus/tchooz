...
class hikamarketMenu_contentType {
	...
	public function display($map, $value, &$js, $id = '', $warning = false) {
		...
		if(!isset($this->values[$value])) {
			...
			return $ret . JHTML::_('select.genericlist', $values, $map, 'class="custom-select" size="1" ' . $options, 'value', 'text', $value, 'content_select');
		}

		return JHTML::_('select.genericlist', $this->values, $map, 'class="custom-select" size="1" ' . $options, 'value', 'text', $value, 'content_select');
	}
}
