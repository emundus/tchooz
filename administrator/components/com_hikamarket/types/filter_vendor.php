...
';
	...
	public function display($map, $value, $invoicemap = '', $invoicevalue = 0) {
		...
		if(is_array($this->values)) {
			$ret = JHTML::_('select.genericlist', $this->values, $map, 'class="custom-select" size="1" data-search-reset="-1" onchange="document.adminForm.submit();"', 'value', 'text', $value);
		} else {
...
window.Oby.ready(function(){ window.oNameboxes["'.$uuid.'"].register("set", function(e){ document.getElementById("'.$uuid.'_text").form.submit(); }); });
		...
		if($value > 1 && !empty($invoicemap)) {
			...
			$ret .= JHTML::_('select.genericlist', $choices, $invoicemap, 'class="custom-select" size="1" onchange="document.adminForm.submit();"', 'value', 'text', $invoicevalue);
		}
