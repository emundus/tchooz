...
<?php
	if(empty($this->orders) && empty($this->vendors)) {
?>
<div id="hikashop_backend_tile_edition">
<div class="hk-row-fluid">
	<em><?php echo JText::_('NO_ORDERS_FOUND'); ?></em>
</div>
</div>
<?php
	} else if(!empty($this->orders)) {
?>
<div id="hikashop_backend_tile_edition">
<div class="hk-row-fluid">
...
window.localPage.getVendorTotal = function(updateCheck) {
	...
	window.localPage.triggerVendorTotal = setTimeout(function() {
		...
		var f = (document.adminForm.form) ? document.adminForm.form : document.adminForm;
		if(updateCheck) {
			f.boxchecked.value = cpt;
		}
		f.boxchecked.dispatchEvent(new CustomEvent('change', {
			bubbles: true,
			cancelable: true
		}));
		var urlTotal = total;
