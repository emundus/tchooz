...
<table class="hk_config_table table" style="width:100%">
	...
	<tr>
		...
		<td>
			HikaMarket <?php echo $this->config->get('level') . ' ' . $this->config->get('version'); ?> [2409092018]
		</td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><?php
			...
			echo JHTML::_('select.genericlist', $values, 'config[product_edit_cancel_url]', 'class="custom-select"', 'value', 'text', $this->config->get('product_edit_cancel_url', 'product'));
		?></td>
	</tr>
	<tr>
	...
	<tr>
		<td class="hk_tbl_key"<?php echo $this->docTip('default_filter_type_product_listing');?>><?php echo JText::_('HIKAM_FRONT_PRODUCT_DEFAULT_LISTING_FILTER'); ?></td>
		<td><?php
			$values = array(
				JHTML::_('select.option', 0, JText::_('DIRECT SUB ELEMENTS')),
				JHTML::_('select.option', 1, JText::_('ALL SUB ELEMENTS')),
			);
			echo JHTML::_('select.genericlist', $values, 'config[default_filter_type_product_listing]', 'class="custom-select"', 'value', 'text', (int)$this->config->get('default_filter_type_product_listing', 0));
		?></td>
	</tr>
	<tr>
...
<tr>
	...
	<td><?php
		echo JHTML::_('hikaselect.booleanlist', 'config[front_small_editor]', 'class="custom-select"', $this->config->get('front_small_editor',0));
	?></td>
</tr>
<tr>
...
<table style="margin-top:5px" class="adminlist table table-striped table-hover admintable">
	<thead>
		<tr>
			...
			<th class="title"><?php echo JText::_('HIKAM_STAT_PERIOD'); ?></th>
...
?>		<tr class="row<?php echo $k ;?>" data-hkm-stat="<?php echo $key; ?>">
			...
			<td><?php
				...
				echo JHTML::_('select.genericlist', $statisticsContainers, 'config[vendor_statistics]['.$key.'][container]', 'class="custom-select"', 'value', 'text', $statistic['container']);
			?></td>
			<td><?php
				if(isset($statistic['vars']['DATE_RANGE']) && $statistic['type'] != 'graph') {
					echo JHTML::_('select.genericlist', $statisticsPeriods, 'config[vendor_statistics]['.$key.'][vars][DATE_RANGE]', 'class="custom-select"', 'value', 'text', $statistic['vars']['DATE_RANGE']);
				} else if(isset($statistic['vars']['DATE_RANGE'])) {
					echo JHTML::_('select.genericlist', $statisticsGroupPeriods, 'config[vendor_statistics]['.$key.'][vars][DATE_RANGE]', 'class="custom-select"', 'value', 'text', $statistic['vars']['DATE_RANGE']);
				}
			?></td>
			<td>
