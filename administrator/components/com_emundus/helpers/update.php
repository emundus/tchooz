...
class EmundusHelperUpdate
{
	...
	public static function getEmundusPlugins()
	{
		...
		{
			echo $e->getMessage();
			Log::add('Failed to retrieve emundus plugins', Log::WARNING, 'com_emundus.error');
		}
				...
				if (!empty($old_events->event))
				{
					...
					if (!empty($events) && !empty($old_events->code))
					{
						...
						foreach ($events as $key => $event)
						{
							...
							{
								Log::add('Failed to backup events', Log::WARNING, 'com_emundus.cli');
							}
	...
	public static function updateCampaignWorkflowTable(): array
	{
		...
		Log::addLogger(['text_file' => 'com_emundus.cli.php'], Log::ALL, array('com_emundus.cli'));
		...
		if (empty($output_status_column->Field))
		{
			...
			{
				Log::add('Error trying to get emundus campaign workflow rows', Log::ERROR, 'com_emundus.cli');
				$error = true;
			}
			...
			if (!$error)
			{
				...
				{
					Log::add('Could not retrieve jos_emundus_campaign_workflow fabrik group and list ids', Log::ERROR, 'com_emundus.cli');
				}
				...
				if (!empty($group_id) && !empty($list_id))
				{
					...
					{
						Log::add('Could not retrieve fabrik element id from name status and group_id ' . $group_id . ' : ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
					}
					...
					if (!empty($status_element_id))
					{
						...
						{
							$updated = false;
							Log::add('Error trying to update element ' . $status_element_id . ' params from group ' . $group_id, Log::ERROR, 'com_emundus.cli');
						...
						if ($updated)
						{
							...
							{
								Log::add('Error trying to create jos_emundus_campaign_workflow_repeat_entry_status ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
							...
							{
								Log::add('Cannot update fabrik element join with new table jos_emundus_campaign_workflow_repeat_entry_status ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
					...
					if (!empty($campaign_element_id))
					{
						...
						{
							$updated = false;
							Log::add('Error trying to update campaign element params from group ' . $group_id . ' : ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
						...
						if ($updated)
						{
							...
							{
								Log::add('Error trying to create jos_emundus_campaign_workflow_repeat_campaign ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
							...
							{
								Log::add('Cannot update fabrik element join with new table jos_emundus_campaign_workflow_repeat_campaign ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
						...
						{
							Log::add('campaign element from fabrik group ' . $group_id . ' has not been updated', Log::WARNING, 'com_emundus.cli');
					...
					catch (Exception $e)
					{
						...
						Log::add('Error trying to insert output_status element in jos_fabrik_elements ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
					...
					if ($output_status_inserted)
					{
						...
						{
							Log::add('Error on output_status creation ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
						...
						{
							Log::add('Error on status column change ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
						...
						if ($every_alter_succeed)
						{
							if (!empty(!empty($old_workflows)))
							{
								...
								Log::add('JSON Save values : ' . json_encode($old_workflows), Log::INFO, 'com_emundus.cli');
								...
								if ($backup_file)
								{
									...
									{
										$update = false;
										Log::add('Failed to set null values on campaign and entry_status columns ' . $e->getMessage(), Log::ERROR, 'com_emundus');
									...
									foreach ($old_workflows as $workflow)
									{
										if (!empty($workflow->id))
										{
											...
											catch (Exception $e)
											{
												...
												Log::add('Failed to join new row in jos_emundus_campaign_workflow_repeat_campaign. ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
											...
											catch (Exception $e)
											{
												...
												Log::add('Failed to join new row in jos_emundus_campaign_workflow_repeat_entry_status. ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
								...
								{
									Log::add('Unable to save old workflows to backup file, stop deletion', Log::ERROR, 'com_emundus.cli');
					...
					{
						Log::add('output_status element in jos_fabrik_elements has not been inserted', Log::WARNING, 'com_emundus.cli');
						$update_campaign_workflow['message'] = 'output_status element in jos_fabrik_elements has not been inserted';
					}
				}
				else
				{
					Log::add('Did not find group_id nor list_id', Log::WARNING, 'com_emundus.cli');
	...
	public static function addProgramToCampaignWorkflow(): array
	{
		...
		if (empty($programs->Field))
		{
			...
			{
				$added = false;
				Log::add('Failed to add programs column to campaign workflow' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
			...
			if ($added)
			{
				...
				{
					Log::add('Could not retrieve jos_emundus_campaign_workflow fabrik group and list ids ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
				}
				...
				if (!empty($group_id) && !empty($list_id))
				{
					...
					catch (Exception $e)
					{
						...
						Log::add('Failed to create fabrik_element \'programs\' ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
					...
					if ($element_created && !empty($program_element_id))
					{
						...
						if ($table_created)
						{
							...
							{
								$joined = false;
								Log::add('Failed to update fabrik element join with new table jos_emundus_campaign_workflow_repeat_programs ' . $e->getMessage(), Log::ERROR, 'com_emundus.cli');
		...
		if (!empty($events))
		{
			...
			foreach ($events as $event)
			{
				if (!empty($event['label']))
				{
					...
					{
						Log::add('Failed to check if event does not already exists ' . $e->getMessage(), Log::ERROR, 'com_emundus.error');
						$event_id = 0;
					}
			...
			if (!empty($rows))
			{
				...
				{
					$inserted = false;
					Log::add('Failed addCustomEvents ' . $e->getMessage(), Log::ERROR, 'com_emundus.error');
				}

				if (!$inserted)
				{
					$response['message'] = 'Custom events have not been inserted in database';
					Log::add('Custom events have not been inserted in database', Log::WARNING, 'com_emundus.error');
				}
				else
				{
	...
	public static function getModule($id = 0, $title = '')
	{
		...
		if (!empty($id) || !empty($title))
		{
			...
			{
				Log::add('Failed to get module ' . $e->getMessage(), Log::ERROR, 'com_emundus.error');
			}
		}
	...
	public static function updateContentTable($id , $value ,$attr)
	{
		...
		{
			Log::add('Failed to update the content with the id ' . $id . ' with value ' . $value . ': ' . $e->getMessage(), Log::ERROR, 'com_emundus.error');
		}
...
<p>Ci-joint, une archive des informations qui ont &eacute;t&eacute; supprim&eacute;es.</p>'),
		...
		if (!empty($scp_plugin))
		{
			...
			$storage_value['base64_exceptions']                = 'com_hikashop,com_emundus,com_fabrik,com_users';
			...
			$storage_value['duplicate_backslashes_exceptions'] = 'com_emundus,com_fabrik,com_content,com_languages,com_users,com_login,com_hikashop';
