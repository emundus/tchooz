<?php
/**
 * eMundus: Postflight Tasks
 *
 * Holds every method invoked by the component installer's postflight step.
 * The installer script discovers them through reflection on
 * #[PostflightAttribute] and calls each one in turn.
 *
 * @package     Joomla
 * @subpackage  eMundus
 * @author      eMundus
...
class Com_EmundusPostflightTasks
{
	...
	public function recompileGantry5(): bool
	{
	...
	public function setSitename(): bool
	{
		$updated = false;
	...
	public function syncGantryLogo(): bool
	{
		$synced = false;
	...
	public function setAdminColorPalette(): bool
	{
	...
	public function checkHtAccess(): bool
	{
	...
	public function checkPasswordFields(): bool
	{
		$checked = true;
	...
	public function checkFabrikIcon(): bool
	{
		$checked = true;
	...
	public function checkFabrikTemplate(): bool
	{
		$checked = true;
		$query   = $this->db->getQuery(true);
	...
	public function convertDateToJDate(): bool
	{
		$converted = true;
	...
	public function checkSSOAvailable(): bool
	{
		$checked = true;
		$haveSSO = false;
	...
	public function checkAutomatedUser(): bool
	{
	...
	public function forceHikashopLightMode(): bool
	{
		$query = $this->db->getQuery(true);
	...
	public function checkSessionGCScheduler(): bool
	{
		$checked = false;
	...
	public function disableDropfilesEditorPlugin(): bool
	{
		$query = $this->db->getQuery(true);
	...
	public function checkHistoryMenu(): bool
	{
		$query = $this->db->getQuery(true);
	...
	public function secureAPI(): bool
	{
	...
	public function secureSomeArticles(): bool
	{
		$query  = $this->db->createQuery();
		$tasks = [];
	...
	public function removeOldAssets(): bool
	{
	...
	public function removeAjaxValidation(): bool
	{
		$removed = true;
	...
	public function checkProfileMenuTranslation(): bool
	{
		$checked = true;
		$query   = $this->db->getQuery(true);
	...
	public function checkEmundusEventHandler(): bool
	{
		$checked = true;
		$query   = $this->db->getQuery(true);
	...
	public function checkZipPlugin(): bool
	{
		$checked = true;
		$query   = $this->db->getQuery(true);
...
<p>Ci-joint, une archive des informations qui ont &eacute;t&eacute; supprim&eacute;es.</p>';
	...
	public function checkDropfilesPlugin(): bool
	{
		$query   = $this->db->getQuery(true);
	...
	public function checkSCPConfiguration(): bool
	{
		$checked = true;
		$query   = $this->db->getQuery(true);
	...
	public function checkFavicon(): bool
	{
	...
	public function checkRegistrationFormAutologin(): bool
	{
		$checked = true;
		$query   = $this->db->getQuery(true);
	...
	public function checkPageClass(): bool
	{
		$checks = [];
	...
	public function checkFabrikDebug(): bool
	{
		$checked = false;
	...
	public function updateResetPasswordBody(): bool
	{
		$inserts = [];
	...
	public function repairCommentsTable(): bool
	{
		$repaired = true;
	...
	public function buildSetupActionsTable(): bool
	{
	...
	/**
	 * Rebuild the "actions" menu type from the canonical enum definition.
	 *
	 * Guarantees for every baseline entry described in {@see MenuHeadingEnum}
	 * and {@see ActionMenuEnum}:
	 *  - FR title is the enum's French label, EN title is synced via Falang.
	 *  - Each action menu is attached to the heading declared on the enum.
	 *  - Action menus appear in the order returned by ActionMenuEnum::byHeading().
	 *
	 * Entries whose alias/link are not part of the enum are left untouched.
	 */
	#[PostflightAttribute(name: "Build actions menu")]
	public function buildActionsMenus(): bool
	{
		try
		{
			$headingIds = $this->syncHeadingMenus();
			$actionIds  = $this->syncActionMenus();

			$this->reorderActionMenus($headingIds, $actionIds);
		}
		catch (\Throwable $e)
		{
			EmundusHelperUpdate::displayMessage($e->getMessage(), 'error');
	...
	public function checkPublicAccess(): bool
	{
		...
		if ($addon->getParam('has_been_activated_once', 0) == 1) { // no turning back now
			...
			$checked = $enabled && !empty($systemUserId);
		}
		else
		{
			$checked = true;
	...
	/**
	 * Update heading menu titles (FR + Falang EN) and return a
	 * MenuHeadingEnum::value => menu id map for the headings found in DB.
	 *
	 * @return  array<string, int>
	 */
	private function syncHeadingMenus(): array
	{
		$query = $this->db->createQuery();
		$query->select($this->db->quoteName(['id', 'title', 'alias']))
			->from($this->db->quoteName('#__menu'))
			->where($this->db->quoteName('menutype') . ' = ' . $this->db->quote('actions'))
			->where($this->db->quoteName('type') . ' = ' . $this->db->quote('heading'));
		$this->db->setQuery($query);

		$rows       = $this->db->loadObjectList('alias') ?: [];
		$headingIds = [];

		foreach ($rows as $alias => $row)
		{
			$heading = MenuHeadingEnum::fromAlias((string) $alias);
			if ($heading === null)
			{
				continue;
			}

			$headingIds[$heading->value] = (int) $row->id;

			$row->title = $heading->getLabelFr();
			$this->db->updateObject('#__menu', $row, 'id');

			EmundusHelperUpdate::insertFalangTranslation(
				1,
				(int) $row->id,
				'menu',
				'title',
	...
	/**
	 * Update action menu titles (FR + Falang EN) and return an
	 * ActionMenuEnum::value => menu id map for the entries found in DB.
	 *
	 * @return  array<string, int>
	 */
	private function syncActionMenus(): array
	{
		$query = $this->db->createQuery();
		$query->select($this->db->quoteName(['id', 'title', 'alias', 'link', 'parent_id']))
			->from($this->db->quoteName('#__menu'))
			->where($this->db->quoteName('menutype') . ' = ' . $this->db->quote('actions'))
			->where($this->db->quoteName('type') . ' <> ' . $this->db->quote('heading'));
		$this->db->setQuery($query);
		...
		foreach ($rows as $row)
		{
			$action = ActionMenuEnum::fromLink((string) $row->link);
			if ($action === null)
			{
				continue;
			}

			$actionIds[$action->value] = (int) $row->id;

			$row->title = $action->getLabelFr();
			$this->db->updateObject('#__menu', $row, 'id');
	...
	/**
	 * Reparent each action menu under its heading and reorder siblings to
	 * match ActionMenuEnum ordering. The first entry is placed as the
	 * first child of the heading, every subsequent entry as "after" its
	 * previously placed sibling — which together yields the desired order.
	 *
	 * @param   array<string, int>  $headingIds  MenuHeadingEnum::value => menu id
	 * @param   array<string, int>  $actionIds   ActionMenuEnum::value  => menu id
	 */
	private function reorderActionMenus(array $headingIds, array $actionIds): void
	{
		if (empty($headingIds) || empty($actionIds))
		{
			return;
		}
		...
		{
			$parentId = $headingIds[$heading->value] ?? null;
			if ($parentId === null)
			{
				continue;
			}
			...
			foreach (ActionMenuEnum::byHeading($heading) as $action)
			{
				$menuId = $actionIds[$action->value] ?? null;
				if ($menuId === null)
				{
					continue;
				}

				$menuTable = Nested::getInstance('Menu');
				if (!$menuTable->load($menuId))
				{
					continue;
				}
				...
				{
					$menuTable->setLocation($parentId, 'first-child');
				}
				else
				{
					$menuTable->setLocation($previousSiblingId, 'after');
				}

				if (!$menuTable->check() || !$menuTable->store())
				{
					Log::add(
						'Failed to move actions menu #' . $menuId . ' (' . $action->name . '): ' . $menuTable->getError(),
						Log::ERROR,
						'com_emundus.error'
					);
					continue;
