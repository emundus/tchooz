...
class EmundusAdminModelSamples extends JModelList
{
	...
	public function createSampleUser($profile = 9)
	{
		...
		$query->clear()
			->insert('#__users')
			->columns('name, username, email, password, registerDate, params')
			->values($db->quote('Test USER') . ', ' . $db->quote($username) . ',' . $db->quote($username . '@emundus.fr') . ',' . $db->quote(md5('test1234')) . ',' . $db->quote(date('Y-m-d H:i:s')) . ',' . $db->quote('{}'));
	...
	public function createSampleProgram($label, $code)
	{
		...
		if (empty($existing)) {
			...
			$query->clear()
				...
				->columns($db->quoteName(array_keys($inserting_datas)))
				->values(implode(',', array_values($inserting_datas)));
			$db->setQuery($query);
			$db->execute();

			$inserting_datas = [
				'parent_id' => 1,
				'course'    => $db->quote($code),
			];
			$query->clear()
			...
			$end_date->modify('+1 year');
			$inserting_datas = [
				'label'      => $db->quote('2022-2023'),
				'schoolyear' => $db->quote('2022-2023'),
				'code'       => $db->quote($code),
				'published'  => 1,
				'date_start' => $db->quote(date('Y-m-d H:i:s')),
				'date_end'   => $db->quote($end_date->format('Y-m-d H:i:s')),
				'profile_id' => 9,
			];
			$query->clear()
