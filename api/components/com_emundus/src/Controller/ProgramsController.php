...
class ProgramsController extends ApiController
{
     ...
     * @since  4.0.0
     */
    protected $contentType = 'programs';
    ...
    public function displayList()
    {
	    ...
	    if (\array_key_exists('code', $apiFilterInfo)) {
		    $this->modelState->set('filter.code', $filter->clean($apiFilterInfo['code'], 'STRING'));
	    }
	    ...
	    if (\array_key_exists('category', $apiFilterInfo)) {
		    $this->modelState->set('filter.category', $filter->clean($apiFilterInfo['category'], 'STRING'));
	    }
