...
class pluginmarketViewpluginmarket extends hikamarketView {
	...
	public function listing($tpl = null) {
		...
		$this->toolbar = array(
			array(
				...
				'name' => JText::_('HIKA_BACK'), 'url' => hikamarket::completeLink('vendor'.$this->url_itemid)
			),
			array(
				'icon' => 'new',
				'fa' => 'fa-plus-circle',
				'name' => JText::_('HIKA_NEW'),
				'url' => hikamarket::completeLink('plugin&plugin_type='.$type.'&task=add'.$this->url_itemid),
				'pos' => 'right',
	...
	public function form($tpl = null) {
		...
		$this->toolbar = array(
			'back' => array(
				...
				'url' => hikamarket::completeLink('plugin&plugin_type='.$type.$this->url_itemid)
			),
			'apply' => array(
				'url' => '#apply',
				'fa' => 'fa-check-circle',
	...
	protected function loadPluginTemplate($view = '', $type = '') {
		...
		$html = $this->processConfig($this->data['pluginConfig'], $type, $paramsType, @$this->element);
		return $html;
	}
	...
	protected function processConfig($configs, $type, $paramsType, $elementData, $id = '', $checkDisplay = false) {
		$html = '';
		if(empty($configs))
			return $html;
		...
		$configData = (!empty($paramsType) && isset($elementData->$paramsType)) ? $elementData->$paramsType : $elementData;
			...
			switch($config['type']) {
				...
				default:
					...
					if(!empty($this->plugin) && method_exists($this->plugin, 'pluginConfigDisplay'))
						$ret = $this->plugin->pluginConfigDisplay($config['type'], $config['data'], $type, $localType, $key, $elementData);
