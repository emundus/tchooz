...
<?php
	foreach($this->breadcrumb as $i => $breadcrumb) {
		...
		if($breadcrumb->category_id != $this->cid) {
			echo '<a href="'.hikamarket::completeLink('category&task=listing&cid='.$breadcrumb->category_id.$this->url_itemid).'">'.JText::_($breadcrumb->category_name).'</a>';
		} else {
...
otreeCategories.callbackSelection = function(tree,id) {
	...
	if(node.value && node.name) {
		var u = "<?php echo hikamarket::completeLink('category&task=listing&cid=HIKACID'.$this->url_itemid, false, false, true); ?>";
...
if(!empty($this->categories)) {
	foreach($this->categories as $category) {
		$url = ($this->manage) ? hikamarket::completeLink('category&task=edit&cid='.(int)$category->category_id.$this->url_itemid) : null;
		$browse_url = hikamarket::completeLink('category&task=listing&cid='.(int)$category->category_id.$this->url_itemid);
		$extra_classes = '';
?>
...
<form action="<?php echo hikamarket::completeLink('category&task=delete'.$this->url_itemid); ?>" method="post" name="hikamarket_delete_category_form" id="hikamarket_delete_category_form">
	<input type="hidden" name="option" value="<?php echo HIKAMARKET_COMPONENT; ?>" />
