	...
	<dl class="hikam_options large">
		...
		<dd class="hikamarket_order_customer_name"><span id="hikamarket_order_customer_name"><?php
			...
			if($this->order->order_user_id > 0 && hikamarket::acl('user/show')) {
				?><a onclick="return window.orderMgr.showCustomer(this);" data-popup-href="<?php echo hikamarket::completeLink('user&task=show&cid='.$this->order->order_user_id.$this->url_itemid, true); ?>" href="<?php echo hikamarket::completeLink('user&task=show&cid='.$this->order->order_user_id.$this->url_itemid); ?>"><?php
					echo $customer_name;
				?></a><?php
			} else
				echo $customer_name;
