...
<form action="<?php echo hikamarket::completeLink('discount'.$this->url_itemid);?>" method="post" name="hikamarket_form" id="hikamarket_discount_form">
...
<?php }
	...
	if(hikashop_level(1) && hikamarket::acl('discount/edit/shippingpercent')) { ?>
		<dt data-discount-display="coupon" class="hikamarket_discount_shipping_percent"><label><?php echo JText::_('DISCOUNT_SHIPPING_PERCENTAGE'); ?></label></dt>
		<dd data-discount-display="coupon" class="hikamarket_discount_shipping_percent">
			<input type="text" size="30" name="data[discount][discount_shipping_percent]" value="<?php echo $this->escape(@$this->discount->discount_shipping_percent); ?>" />%
		</dd>
<?php }
