...
class productMarketController extends hikamarketController {
	protected $rights = array(
		...
		'modify' => array('apply', 'save', 'save_translation', 'copy', 'toggle', 'save2new'),
		'delete' => array('delete')
	);
	...
	protected $ordering = array(
		'type' => 'product',
		'pkey' => 'product_category_id',
		'main_pkey' => 'product_id',
	...
	public function authorize($task) {
		if($task == 'toggle' || $task == 'delete') {
			...
			if(!JSession::checkToken('request'))
				return false;
			if($task == 'toggle' && !$this->isNewProduct($product_id) && !hikamarket::acl('product/edit/published'))
				return false;
			if($task == 'toggle' && $this->isNewProduct($product_id) && !hikamarket::acl('product/new/published'))
				return false;
	...
	public function save() {
		...
		if(!empty($cancel_action) && !empty($product_id)) {
			switch($cancel_action) {
				case 'product':
					$app->redirect( hikamarket::completeLink('shop.product&task=show&cid='.$product_id.$url_itemid, false, true) );
					break;
				case 'url':
		...
		$app->redirect( hikamarket::completeLink('product&task=listing'.$url_itemid, false, true) );
		return false;
	}
	...
	public function edit() {
		...
		if( !$this->isNewProduct($product_id) && !hikamarket::acl('product/edit') )
			return hikamarket::deny('product', JText::sprintf('HIKAM_ACTION_DENY', JText::_('HIKAM_ACT_PRODUCT_EDIT')));
		if( $this->isNewProduct($product_id) && !hikamarket::acl('product/new') )
			return hikamarket::deny('product', JText::sprintf('HIKAM_ACTION_DENY', JText::_('HIKAM_ACT_PRODUCT_EDIT')));
	...
	public function variant() {
		...
		if( !hikamarket::loginVendor() )
			return false;
		$product_id = hikamarket::getCID('product_id');
		if( !$this->isNewProduct($product_id) && !hikamarket::acl('product/edit/variants') )
			return hikamarket::deny('product', JText::sprintf('HIKAM_ACTION_DENY', JText::_('HIKAM_ACT_PRODUCT_EDIT')));
		if( $this->isNewProduct($product_id) && !hikamarket::acl('product/new/variants') )
			return hikamarket::deny('product', JText::sprintf('HIKAM_ACTION_DENY', JText::_('HIKAM_ACT_PRODUCT_EDIT')));
		...
		if(in_array(hikaInput::get()->getCmd('tmpl', ''), array('component','raw'))) {
	...
	public function add() {
		...
		if( !hikamarket::loginVendor() )
			return false;
		if( !hikamarket::acl('product/add') || (!hikamarket::acl('product/edit') && !hikamarket::acl('product/new')) )
			return hikamarket::deny('product', JText::sprintf('HIKAM_ACTION_DENY', JText::_('HIKAM_ACT_PRODUCT_EDIT')));
...
';
	...
	public function addimage() {
		...
		if( !hikamarket::loginVendor() )
			return false;
		if( !hikamarket::acl('product/edit') && !hikamarket::acl('product/new') )
			return hikamarket::deny('product', JText::sprintf('HIKAM_ACTION_DENY', JText::_('HIKAM_ACT_PRODUCT_EDIT')));
