...
//                }
    ...
    public function searchFile($data)
    {
        ...
        if (!isset($data['adminsearch']) && $count === 0) {
            return $results;
        }
    ...
    public function getLatestFiles($data_cat, $data)
    {
        ...
        foreach ($data_cat as $key => $catid) {
            ...
            if (is_numeric($catid)) {
                ...
                } elseif ($cat_typle === 'onedrivebusiness') {
                    $conditionDropbox .= ' AND a.catid = ' . $dbo->quote($catid) . '';

                    $queryOnedrive = 'SELECT a.* FROM #__dropfiles_onedrive_business_files as a';
                    $queryOnedrive .= ' WHERE ' . $conditionDropbox . ' ORDER BY ' . $data['fOrderType'];
                    $queryOnedrive .= ' DESC ' . $limit;
                ...
                $resultOnedriveBusiness = array();
                if (!is_null($params['onedriveBusinessConnected']) && (int) $params['onedriveBusinessConnected'] === 1) {
                    $queryOnedrive = 'SELECT a.* FROM #__dropfiles_onedrive_business_files as a ';
                    $queryOnedrive .= '  WHERE ' . $conditionDropbox . ' ORDER BY ' . $data['fOrderType'];
                    $queryOnedrive .= ' DESC ' . $limit;

                    $resultOnedriveBusiness = $this->searchFileLocal($queryOnedrive);

                    if (!empty($resultOnedriveBusiness)) {
                        foreach ($resultOnedriveBusiness as $item) {
                            $item->id = $item->file_id;
                            $item->catid = $this->getOneCatLocalByCloudID($item->catid);
                            if (!$item->catid) {
                                continue;
                ...
                $array1 = array_merge($array1, $resultOnedriveBusiness);
