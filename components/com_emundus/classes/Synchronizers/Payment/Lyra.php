...
abstract class Lyra implements PaymentSynchronizerInterface
{
	...
	abstract protected function getPaymentGateway(): PaymentGatewayEnum;

	public function __construct()
	{
		$this->logChannel = 'com_emundus.' . $this->getPaymentGateway()->value;
		Log::addLogger(['text_file' => $this->logChannel . '.php',], Log::ALL, [$this->logChannel]);
		...
		try {
			...
			Log::add('Error on ' . $this->getPaymentGateway()->value . ' connection : ' . $e->getMessage(), Log::ERROR, $this->logChannel);
		}
	}
	...
	private function getAuthenticationInfos(): array
	{
		...
		if (empty($auth['client_id']) || empty($auth['client_secret'])) {
			Log::add('No authentication available', Log::ERROR, $this->logChannel);
	...
	private function setConfig(): void
	{
		...
		$query->select('config')
			...
			->where('type like ' . $this->db->quote($this->getPaymentGateway()->value));
		...
		try {
			...
			Log::add('Error on get ' . $this->getPaymentGateway()->value . ' api config : ' . $e->getMessage(), Log::ERROR, $this->logChannel);
		}
	}
	...
	private function setEndpoint(): void
	{
		...
		if (empty($endpoint)) {
			Log::add('No endpoint available', Log::ERROR, $this->logChannel);
	...
	/**
	 * Method provided by Lyra to generate the signature
	 * @param   array   $params
	 * @param   string  $key
	 *
	 * @return string
	...
	/**
	 * Lyra needs an alphanumeric string of 6 characters
	...
	public function verifyReference(string $external_reference): bool
	{
		...
		if (!empty($external_reference)) {
			...
			if (strlen($external_reference) <= 6 && ctype_alnum($external_reference)) {
				...
				Log::add(Text::_('COM_EMUNDUS_ERROR_INVALID_EXTERNAL_REFERENCE'), Log::WARNING, $this->logChannel);
			}
		}
	...
	private function prepareDefaultFields(TransactionEntity $transaction, CartEntity $cart): array
	{
		...
		$fields = [
			...
			'vads_amount' => (int) round($transaction->getAmount() * 100),
			...
			'vads_cust_address' => $address !== null ? ($address->getStreetAddress() ?? '') : '',
			'vads_cust_address2' => $address !== null ? ($address->getExtendedAddress() ?? '') : '',
			'vads_cust_city' => $address !== null ? ($address->getLocality() ?? '') : '',
			'vads_cust_zip' => $address !== null ? ($address->getPostalCode() ?? '') : '',
			'vads_cust_country' => $address !== null ? $this->getCountryISO2($address->getCountry() ?? 0) : '',
	...
	private function prepareImmediateTransaction(TransactionEntity $transaction, CartEntity $cart): array
	{
	 ...
	 * @throws \Exception
	 */
	private function prepareInstallmentTransaction(TransactionEntity $transaction, CartEntity $cart): array
	{
		$fields = $this->prepareDefaultFields($transaction, $cart);
		...
		$fields['vads_sub_amount'] = (int) round($amount_by_date * 100);
		...
		if ($amounts_by_iterations[0] !== end($amounts_by_iterations)) {
			$fields['vads_sub_init_amount'] = (int) round($amounts_by_iterations[0] * 100);
	...
	public function prepareCheckout(TransactionEntity $transaction, CartEntity $cart): array
	{
		...
		{
			case 'sepa':
				$fields = $this->prepareInstallmentTransaction($transaction, $cart);
				break;
			default:
				$fields = $this->prepareImmediateTransaction($transaction, $cart);
				break;
		}
		...
		if (!empty($transaction_id) && !empty($data)) {
			...
			if (!empty($transaction) && $transaction->getExternalReference() === $data['vads_trans_id'])
			{
				...
				switch($data['vads_trans_status']) {
					...
					case 'ACCEPTED':
					case 'AUTHORISED':
						$new_status = TransactionStatus::CONFIRMED;
						break;
					case 'CAPTURE_FAILED':
					...
					case 'WAITING_AUTHORISATION_TO_VALIDATE':
						$new_status = TransactionStatus::WAITING;
						break;
				}

				if ($new_status === null) {
					Log::add('Ignoring ' . $this->getPaymentGateway()->value . ' callback for transaction ' . $transaction->getId() . ' : unknown vads_trans_status "' . $data['vads_trans_status'] . '"', Log::WARNING, $this->logChannel);

					return false;
				}

				if ($transaction->getStatus() === TransactionStatus::CONFIRMED && $new_status !== TransactionStatus::CONFIRMED) {
					Log::add('Ignoring ' . $this->getPaymentGateway()->value . ' callback for transaction ' . $transaction->getId() . ' : already CONFIRMED, refusing downgrade to ' . $new_status->value . ' (vads_trans_status=' . $data['vads_trans_status'] . ')', Log::WARNING, $this->logChannel);
				...
				if ($updated)
				{
					Log::add('Transaction ' . $transaction->getId() . ' updated successfully from ' . $this->getPaymentGateway()->value . ' callback', Log::INFO, $this->logChannel);
				}
				else
				{
					Log::add('Failed to update transaction ' . $transaction->getId() . ' from ' . $this->getPaymentGateway()->value . ' callback', Log::ERROR, $this->logChannel);
				}
			} else {
				Log::add('Transaction not found or external reference mismatch on ' . $this->getPaymentGateway()->value . ' callback', Log::ERROR, $this->logChannel);
			}
		}
