...
/**
 * Legacy Excel export (export_version "default").
 *
 * Isolated verbatim from ExcelService during Phase 1 of the export refactor
 * (cf. ARCHITECTURE.md). Frozen: no new feature goes here. Kept because the
 * "default" export version may still be triggered by older views / the API.
 *
 * @deprecated Use ExcelService (export_version "next") instead.
 */
class LegacyExcelService extends Export implements ExportInterface
{
	private array $fnums;
	...
	public function exportOld(string $exportPath, array $metadata): ExportResult
	{
		...
		if ($result->getProgress() === 100.00)
		{
			...
			if (!$exportPath = ExcelService::convertToXlsx($csvPath, $excel_filename, $exportPath, $totalfile, $nbcol))
			{
