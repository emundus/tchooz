...
class ObjectList implements ObjectInterface
{
	...
	{
		return Text::_('COM_EMUNDUS_CONTENTELEMENTS_EVAL_FORMS');
	}

	public function getDescription(): string
	{
		return Text::_('COM_EMUNDUS_CONTENTELEMENTS_EVAL_FORMS_DESC');
	}
	...
	public function getDatas(array $filters = []): array
	{
		try
		{
			...
			{
				$query->where($db->quoteName(('fl.'.$filter)) . ' = 1');
			}
			$query->where($db->quoteName('fl.db_table_name') . ' LIKE ' . $db->quote('jos_emundus_evaluations_%'));
			...
			{
				$data->label = LanguageFactory::getTranslation($data->label);
			}
		}
		catch (\Exception $e)
		{
		...
		try
		{
			...
			if ($table == 'fabrik_forms')
			{
				$query->select('id')
					...
					->where($db->quoteName('id') . ' = ' . (int) $reference_id);
				$db->setQuery($query);
				$forms = $db->loadColumn();
			}

			$query->clear()
				->select('id,' . $db->quoteName($label) . ' as label')
			...
			{
				$children->label = LanguageFactory::getTranslation($children->label);
			}
		}
		catch (\Exception $e)
		{
