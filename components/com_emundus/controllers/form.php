			...
			foreach ($data['datas'] as $key => $form)
			{
				...
				if (!empty($campaigns))
				{
					...
					else
					{
						...
						$short_tags .= '<span class="tw-cursor-pointer tw-text-profile-full tw-flex tw-items-center tw-justify-center hover:tw-underline">' . count($campaigns) . Text::_('COM_EMUNDUS_ONBOARD_CAMPAIGNS_ASSOCIATED') . '</span>';
						$short_tags .= '</div>';
						$tags       .= '</div>';
					}
				}
