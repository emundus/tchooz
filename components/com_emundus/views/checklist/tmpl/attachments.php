    ...
    <fieldset class="em-container-attachment">
      ...
      <table width="100%" border="0" id="<?php echo $attachment->id; ?>" class="em-container-attachment-table">
        ...
        <tbody>
          ...
          <tr>
            <td>• <?php echo empty($item->description)?JText::_('COM_EMUNDUS_ATTACHMENTS_NO_DESC'):$item->description; ?></td>
            <td><?php if($item->can_be_viewed==1 || $current_user->profile <= 4) {?>
              <a href="<?php echo $chemin.$user->id.'/'.$item->filename; ?>" target="_blank"><img src="media/com_emundus/images/icones/viewmag_16x16.png" alt="show" style="vertical-align:middle"/><?php echo JText::_('COM_EMUNDUS_ATTACHMENTS_VIEW'); ?></a>
              <?php } else { echo JText::_('COM_EMUNDUS_ATTACHMENTS_CANT_VIEW'); } ?>
              &nbsp;-&nbsp;
              <?php if($item->can_be_deleted==1 || $current_user->profile <= 4) {?>
              <a href="?option=com_emundus&task=delete&layout=attachments&sid=<?php echo $user->id; ?>&aid=<?php echo $item->id; ?>"><img src="media/com_emundus/images/icones/trashcan_full.png"  style="vertical-align:middle" alt="delete"/><?php echo JText::_('COM_EMUNDUS_ACTIONS_DELETE'); ?></a>
              <?php } else { echo JText::_('COM_EMUNDUS_ATTACHMENTS_CANT_DELETE'); } ?></td>
          </tr>
          <?php 	} ?>
        ...
        <?php 	} else { ?>
        <tr>
          <td colspan="2"><p class="description"><?php echo JText::_('COM_EMUNDUS_ATTACHMENTS_NO_MORE').' '.$attachment->value.'<br />'.JText::_('COM_EMUNDUS_ATTACHMENTS_MAX_ALLOWED').' '.$attachment->nbmax; ?></p></td>
        </tr>
        </tbody>
