...
<div id="profilebasics" class="em-container-profiles">
    <h2><?php echo $this->profile->label; ?> <a
                href="index.php?option=com_fabrik&view=list&listid=67"><?php echo '[' . JText::_('COM_EMUNDUS_PROFILES_BACK_TO_PROFILE') . ']'; ?></a>
    </h2>
    ...
    <form action="<?= JUri::base(); ?>/index.php?option=com_emundus&task=updateprofile&rowid=<?php echo JFactory::getApplication()->input->get('rowid', $default = null, $hash = 'GET', $type = 'none', $mask = 0); ?>&Itemid=<?php echo $Itemid; ?>"
        ...
        <h2><?php echo JText::_('COM_EMUNDUS_ATTACHMENTS_ATTACHMENTS'); ?> <a
                    href="index.php?option=com_fabrik&view=list&listid=36"><?php echo '[' . JText::_('COM_EMUNDUS_ATTACHMENTS_SETUP_ATTACHMENTS') . ']'; ?></a>
        </h2>
        <div class="table-responsive">
            <table id="attachmentlist" class="table-striped em-form-documents-table">
				...
				<?php foreach ($this->attachments as $attachment) : ?>
                    <tr>
                        <td>
                            <a href="index.php?option=com_fabrik&view=form&formid=34&rowid=<?php echo $attachment->id; ?>&listid=36">[<?php echo JText::_('COM_EMUNDUS_ACTIONS_EDIT'); ?>
                                ]</a>
							<?php echo $attachment->value; ?> (<?php echo $attachment->allowed_types; ?>)
                            <input type="hidden" name="aid[]" value="<?php echo $attachment->id; ?>"/>
                        </td>
                        <td align="center"><input type="checkbox" name="as[]" value="<?php echo $attachment->id; ?>"
                                                  id="selecteda<?php echo $attachment->id; ?>" <?php if ($attachment->selected > 0) echo 'checked'; ?>
                                                  onClick="javascript:toggleA('<?php echo $attachment->id; ?>')"/>
                        </td>
                        <td align="center"><input type="checkbox" name="ad[]" value="<?php echo $attachment->id; ?>"
                                                  id="displayeda<?php echo $attachment->id; ?>" <?php if ($attachment->displayed > 0) echo 'checked'; ?>/>
                        </td>
                        <td align="center"><input type="checkbox" name="ar[]" value="<?php echo $attachment->id; ?>"
                                                  id="requireda<?php echo $attachment->id; ?>" <?php if ($attachment->mandatory > 0) echo 'checked'; ?>
                                                  onClick="javascript:toggleD('<?php echo $attachment->id; ?>')"/>
                        </td>
                    </tr>
				<?php endforeach; ?>
                <tr>
                    <td colspan="5" align="center"><input type="submit" class="btn btn-warning"
                                                          value="<?php echo JText::_('COM_EMUNDUS_ACCESS_UPDATE'); ?>"/>
                    </td>
                </tr>
                </tbody>
            </table>
