...
$app->getSession()->set('application_layout', 'mail');
        ...
        <div class="panel panel-default widget em-container-mail">
            <div class="panel-heading em-container-mail-heading !tw-bg-profile-full">
                ...
                <h3 class="panel-title">
					...
					<?= Text::_('COM_EMUNDUS_EMAILS_MESSAGES'); ?>
            ...
            <div class="panel-body em-container-mail-body">
				...
				<?php if ($this->messages === false) : ?>
                    <h3> <?= Text::_('COM_EMUNDUS_EMAILS_ERROR_GETTING_MESSAGES'); ?> </h3>
				<?php elseif (count($this->messages) === 0) : ?>
                    ...
                    <p class="tw-text-center"><?php echo Text::_('COM_EMUNDUS_EMAILS_NO_MESSAGES_FOUND'); ?></p>
				<?php else : ?>
                    ...
                    <div class="em-flex-row em-border-bottom-neutral-300 mb-3"
                        ...
                        <div id="tab_link_file" onclick="filterMessages('file')"
                            ...
                            <p class="em-font-size-14 em-neutral-900-color"
                               title="<?= Text::_('COM_EMUNDUS_EMAIL_CURRENT_FILE') ?>"
                               style="white-space: nowrap"><?= Text::_('COM_EMUNDUS_EMAIL_CURRENT_FILE') ?></p>
                        </div>
						<?php if (empty($this->hide_all_files)) : ?>
                        <div id="tab_link_all" onclick="filterMessages('all')"
                             class="em-mr-16 em-flex-row em-light-tabs profile_tab em-pointer mb-2">
                            <p class="em-font-size-14 em-neutral-900-color"
                               title="<?= Text::_('COM_EMUNDUS_EMAIL_ALL_FILES') ?>"
                               style="white-space: nowrap"><?= Text::_('COM_EMUNDUS_EMAIL_ALL_FILES') ?></p>
                        </div>
						<?php endif; ?>
                    </div>
					...
					<?php foreach ($this->messages as $message) :?>
                        <div class='message_<?php echo $message->fnum_to ?> panel panel-default em-container-mail-content'
                            ...
                            <div class="panel-heading em-container-mail-content-heading tw-flex tw-flex-col"><h3
                                ...
                                <small class="tw-mb-1"> <?= Text::_('COM_EMUNDUS_EMAILS_MESSAGE_FROM') . ': ' . JFactory::getUser($message->user_id_from)->name . ' ' . EmundusHelperDate::displayDate($message->date_time, 'DATE_FORMAT_LC2', 0); ?> </small>
								<?php if (!empty($message->fnum_to) && $message->fnum_to != $fnum) : ?>
                                    <small><?= Text::_('COM_EMUNDUS_EMAIL_ON_FILE') ?> <a
							...
							<?php if (!empty($message->email_to)): ?>
                                <div class="panel-body em-container-mail-content-body">
                                    <i><?= Text::_('COM_EMUNDUS_EMAILS_MESSAGE_SENT_TO') . ' ' . $message->email_to; ?></i>
                                </div>
							<?php endif; ?>
							<?php if (!empty($message->email_cc)): ?>
                                <div class="panel-body em-container-mail-content-body">
                                <i><?= Text::_('COM_EMUNDUS_EMAIL_PEOPLE_CC') . ' ' . $message->email_cc; ?></i>
                                </div><?php endif; ?>
                            <div class="panel-body em-container-mail-content-body">
								<?php
								$length = strlen($message->message);
								if ($length > 1000000) {
									echo '<strong>' . Text::sprintf('COM_EMUNDUS_ERROR_TOO_LARGE_EMAIL', $length) . '</strong>';
								}
								else {
...
<script>
    function filterMessages(type) {
        ...
        let selected_tab = document.getElementById("tab_link_" + type);
        selected_tab.classList.add("em-light-selected-tab");
        let fnum = "<?php echo $fnum; ?>";
        let messages_to_display = document.querySelectorAll(".em-container-mail-content");

        if (type === 'file') {
            let messages_to_hide = document.querySelectorAll(".em-container-mail-content");
            messages_to_display = document.querySelectorAll('.message_' + fnum)

            for (let i = 0; i < messages_to_hide.length; i++) {
                messages_to_hide[i].style.display = "none";
            }
        }

        for (let i = 0; i < messages_to_display.length; i++) {
