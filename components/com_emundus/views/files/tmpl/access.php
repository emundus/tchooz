...
<div>
    <div>
		...
		foreach ($this->users as $user) : ?>
            ...
            <h4 title="<?= $user['fnum']; ?>"><?= $user['name']; ?> (<?= $user['label']; ?> - <?= $user['year']; ?>
                )</h4>
                    ...
                    <tr class="em-actions-table-line">
						...
						<?php if ($action['c'] == 1) : ?>
                            <td id="c-check-<?= $action['id']; ?>" class="em-has-checkbox">
                                <input type="checkbox" class="em-modal-check c-check"
                                       name="c-check-<?= $action['id']; ?>"
                                       id="c-check-<?= $action['id']; ?>" <?= (@$this->actions_evaluators->{@$action['id']}->c == 1) ? 'checked' : ''; ?> />
                            </td>
						<?php else : ?>
						...
						<?php if ($action['r'] == 1) : ?>
                            <td id="r-check-<?= $action['id']; ?>" class="em-has-checkbox">
                                <input type="checkbox" class="em-modal-check r-check"
                                       name="r-check-<?= $action['id']; ?>"
                                       id="r-check-<?= $action['id']; ?>" <?= (@$this->actions_evaluators->{@$action['id']}->r == 1) ? 'checked' : ''; ?> />
                            </td>
						<?php else : ?>
						...
						<?php if ($action['u'] == 1) : ?>
                            <td id="u-check-<?= $action['id']; ?>" class="em-has-checkbox">
                                <input type="checkbox" class="em-modal-check u-check"
                                       name="u-check-<?= $action['id']; ?>"
                                       id="u-check-<?= $action['id']; ?>" <?= (@$this->actions_evaluators->{@$action['id']}->u == 1) ? 'checked' : ''; ?> />
                            </td>
						<?php else : ?>
						...
						<?php if ($action['d'] == 1) : ?>
                            <td id="d-check-<?= $action['id']; ?>" class="em-has-checkbox">
                                <input type="checkbox" class="em-modal-check d-check"
                                       name="d-check-<?= $action['id']; ?>"
                                       id="d-check-<?= $action['id']; ?>" <?= (@$this->actions_evaluators->{@$action['id']}->d == 1) ? 'checked' : ''; ?> />
                            </td>
						<?php else : ?>
