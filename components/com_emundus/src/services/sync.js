...
export default {
	...
	async saveConfig(config, type) {
		try {
			...
			return await client().post(`index.php?option=com_emundus&controller=sync&task=saveconfig`, formData, {
				headers: {
					'Content-Type': 'multipart/form-data',
				},
			});
	...
	async getAttachmentAspectsConfig(attachmentId) {
		try {
			const response = await client().get(
				`index.php?option=com_emundus&controller=sync&task=getattachmentaspectsconfig`,
				{
					params: {
	...
	saveAttachmentAspectsConfig(attachmentId, config) {
		try {
			...
			return client().post(`index.php?option=com_emundus&controller=sync&task=saveattachmentaspectsconfig`, formData, {
				headers: {
					'Content-Type': 'multipart/form-data',
				},
			});
	...
	async uploadAspectFile(file) {
		try {
			...
			return await client().post(`index.php?option=com_emundus&controller=sync&task=uploadaspectfile`, formData, {
				headers: {
					'Content-Type': 'multipart/form-data',
				},
			});
	...
	async updateSync(did, sync) {
		try {
			...
			return await client().post(`index.php?option=com_emundus&controller=sync&task=updatedocumentsync`, formData, {
				headers: {
					'Content-Type': 'multipart/form-data',
				},
			});
