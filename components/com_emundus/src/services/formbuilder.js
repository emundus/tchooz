...
export default {
	...
	getDatabaseJoinOrderColumns(databaseName) {
		try {
			return fetch(
				'/index.php?option=com_emundus&controller=formbuilder&task=getDatabaseJoinOrderColumns&database_name=' +
					databaseName,
				{
					method: 'GET',
				},
	...
	async getDocumentSample(documentId, profileId) {
		if (documentId > 0 && profileId > 0) {
			try {
				return fetch(
					'/index.php?option=com_emundus&controller=formbuilder&task=getdocumentsample&document_id=' +
						documentId +
						'&profile_id=' +
						profileId,
					{
	...
	async checkIfModelTableIsUsedInForm(modelId, profileId) {
		...
		if (modelId > 0 && profileId > 0) {
			try {
				return client
					.get('checkifmodeltableisusedinform', {
						model_id: modelId,
	...
	async getSqlDropdownOptions(table, key, value, translate) {
		...
		if (table && key && value) {
			try {
				return fetch(
					'/index.php?option=com_emundus&controller=formbuilder&task=getsqldropdownoptions&table=' +
						table +
						'&key=' +
						key +
						'&value=' +
