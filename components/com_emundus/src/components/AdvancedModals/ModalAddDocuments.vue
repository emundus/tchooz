		...
		>
			...
			<div>
				...
				<div class="tw-mb-4" v-if="can_be_deleted">
					<button type="button" class="tw-btn-tertiary" @click="deleteModel">
						{{ translations.DeleteDocTemplate }}
					</button>
				</div>
				...
				<div class="tw-mb-4" v-if="currentDoc == null">
					...
					>
						...
						<option v-for="(modelT, index) in models" :key="'option_' + index" :value="modelT.id">
							{{ modelT.name[langue] }} ({{ modelT.allowed_types }})
						</option>
					</select>
				</div>
				...
				<div class="form-group">
					...
					<div class="input-can-translate tw-flex tw-items-center tw-justify-between">
						<input
							type="number"
							maxlength="100"
							class="form__input field-general w-input tw-mb-0"
							id="image-min-width"
							min="300"
							v-model="form.minResolution.width"
							style="max-width: 48%"
							@keyup="ZeroOrNegative()"
							v-on:keydown.tab="ZeroOrNegative()"
							:placeholder="translations.MinResolutionPlaceholder"
						/>
						<input
							type="number"
							maxlength="100"
							...
							id="image-max-width"
							min="300"
							v-model="form.maxResolution.width"
							style="max-width: 48%"
							@keyup="ZeroOrNegative()"
							v-on:keydown.tab="ZeroOrNegative()"
							:placeholder="translations.MaxResolutionPlaceholder"
						/>
					</div>
				...
				<div class="form-group">
					...
					<div class="input-can-translate tw-flex tw-items-center tw-justify-between">
						<input
							...
							id="image-min-height"
							min="300"
							v-model="form.minResolution.height"
							style="max-width: 48%"
							@keyup="ZeroOrNegative()"
							v-on:keydown.tab="ZeroOrNegative()"
							:placeholder="translations.MinResolutionPlaceholder"
						...
						<input
							...
							id="image-max-height"
							min="300"
							v-model="form.maxResolution.height"
							style="max-width: 48%"
							@keyup="ZeroOrNegative()"
							v-on:keydown.tab="ZeroOrNegative()"
							:placeholder="translations.MaxResolutionPlaceholder"
			...
			<div class="tw-flex tw-items-center tw-justify-between tw-mb-2">
				...
				<button type="button" class="tw-btn-primary !tw-w-auto" @click.prevent="createNewDocument()">
...
export default {
	...
	methods: {
		...
		createNewDocument() {
			...
			if (
				this.model.allowed_types.includes('jpg') ||
				this.model.allowed_types.includes('jpeg') ||
				this.model.allowed_types.includes('png') ||
				this.model.allowed_types.includes('gif')
			) {
				y.push('jpeg;jpg;png;gif');
			}
			if (
				this.model.allowed_types.includes('xls') ||
				this.model.allowed_types.includes('xlsx') ||
				this.model.allowed_types.includes('odf')
			) {
				y.push('xls;xlsx;odf');
			}
				...
				if (this.form.minResolution.height && this.form.maxResolution.height) {
					if (parseInt(this.form.minResolution.width) >= 300 && parseInt(this.form.maxResolution.width) >= 300) {
						if (parseInt(this.form.minResolution.width) > parseInt(this.form.maxResolution.width)) {
							...
							this.errorWidth.message =
								parseInt(this.form.minResolution.width) <= 0
									? this.translations.ErrorResolutionNegative
									: this.translations.ErrorResolution;
				...
				if (this.form.minResolution.width && this.form.maxResolution.width) {
					if (parseInt(this.form.minResolution.height) >= 300 && parseInt(this.form.maxResolution.height) >= 300) {
						if (parseInt(this.form.minResolution.height) > parseInt(this.form.maxResolution.height)) {
							this.errorHeight.error = true;
							this.errorHeight.message =
								parseInt(this.form.minResolution.height) <= 0
									? this.translations.ErrorResolutionNegative
									: this.translations.ErrorResolution;
							document.getElementById('image-min-height').style.setProperty('border-color', 'red', 'important');
							sendError = true;
	...
	watch: {
		doc: function (val) {
			if (val != null) {
				...
				this.form.selectedTypes['xls;xlsx;odf'] =
					this.model.allowed_types.includes('xls') ||
					this.model.allowed_types.includes('xlsx') ||
					this.model.allowed_types.includes('odf');
