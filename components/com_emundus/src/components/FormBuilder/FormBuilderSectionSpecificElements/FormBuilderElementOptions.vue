...
export default {
	...
	methods: {
		...
		updateOption(index, option, next = false) {
			this.loading = true;
			formBuilderService
				.updateOption(this.element.id, this.element.params.sub_options, index, option, this.shortDefaultLang)
				.then((response) => {
					if (response.status) {
		...
		updateOrder() {
			if (this.arraySubValues.length > 1) {
				...
				if (
					!this.element.params.sub_options.sub_values.every(
						(value, index) => value === sub_options_in_new_order.sub_values[index],
					)
				) {
					this.loading = true;
					formBuilderService
						.updateElementSubOptionsOrder(this.element.id, this.element.params.sub_options, sub_options_in_new_order)
						.then((response) => {
							if (response.status) {
								this.reloadOptions();
							} else {
