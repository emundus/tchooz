	...
	<div id="form-builder-elements" style="min-width: 260px">
		<div class="tw-flex tw-items-center tw-justify-around">
			<div
				...
				:class="selected === menu.id ? 'em-light-selected-tab' : ''"
			>
				{{ translate(menu.name) }}
			</div>
		</div>
					...
					>
						...
						<div class="tw-flex tw-h-[18px] tw-w-[18px] tw-items-center">
							...
							<span
								...
								@click="clickCreateElement(element)"
		...
		publishedElements() {
			...
			if (this.keywords) {
				return this.elements.filter(
					(element) =>
						element.published && this.translate(element.name).toLowerCase().includes(this.keywords.toLowerCase()),
				);
			} else {
