...
import { Chip, Button } from '@emundus/ui';

export default {
	name: 'PollRun',
	components: { Chip, ModalHeader, Button, PollSlotContact },
	mixins: [alerts],
	props: {
		item: Object,
		items: {
	...
	methods: {
		...
		async runPoll() {
			...
			this.isRunning = true;
			try {
				const response = await pollService.runPoll(payload);
...
<template>
	<div class="tw-flex tw-flex-col tw-gap-6">
		<div class="tw-flex tw-flex-col tw-gap-6 tw-p-2">
			<ModalHeader :title="translate('COM_EMUNDUS_POLL_RUN_TITLE')" @close="$emit('close')" />

			<div class="tw-flex tw-flex-col tw-gap-2">
				<label class="tw-text-base tw-font-medium">
					{{ translate('COM_EMUNDUS_POLL_RUN_FIELD_LABEL') }}
				</label>
			...
			<label class="tw-flex tw-cursor-pointer tw-items-center tw-gap-1">
				...
				<span class="tw-text-base tw-font-medium">
					{{ translate('COM_EMUNDUS_POLL_RUN_NOTIFY') }}
				</span>
			</label>
		</div>
		...
		<PollSlotContact
			...
			:default-subject="this.pollAddonConfiguration.configuration.run_email_subject"
			:default-body="this.pollAddonConfiguration.configuration.run_email_body"
			@back="closeContact"
			@send="handleSend"
		/>

		<div class="tw-flex tw-justify-center">
			<Button variant="primary" :disabled="isRunning" @click="runPoll" :loading="isRunning">
				{{ translate(isRunning ? 'COM_EMUNDUS_POLL_RUN_SENDING' : 'COM_EMUNDUS_POLL_RUN_OPEN') }}
			</Button>
		</div>
	</div>
</template>
