...
export default {
	...
	methods: {
		...
		renderChart(filter_key = null) {
			...
			fetch('/index.php?option=com_emundus&controller=dashboard&task=renderchartbytag', {
				method: 'POST',
				body: formData,
			})
						...
						for (const filter of this.dataSource.filters) {
							if (typeof this.selectedFilters[filter.key] == 'undefined' || this.selectedFilters[filter.key] == null) {
								...
								if (this.widget_filters[filter.key]) {
									this.selectedFilters[filter.key] = filter.options.filter((option) =>
										this.widget_filters[filter.key].value.includes(option.value),
									);
								}
							}
						}
		...
		getArticle() {
			fetch(
				'/index.php?option=com_emundus&controller=dashboard&task=getarticle&widget=' +
					this.selectedWidget.id +
					'&article=' +
					this.selectedWidget.article_id,
			)
				.then((response) => response.json())
				.then((data) => {
		...
		getEval() {
			fetch('/index.php?option=com_emundus&controller=dashboard&task=geteval&widget=' + this.selectedWidget.id)
				.then((response) => response.json())
				.then((data) => {
					this.datas = data.data;
				});
		...
		getWidgets() {
			fetch('/index.php?option=com_emundus&controller=dashboard&task=getwidgets&all=true')
				.then((response) => response.json())
				.then((data) => {
		...
		updateDashboard() {
			...
			fetch('/index.php?option=com_emundus&controller=dashboard&task=updatemydashboard', {
				method: 'POST',
				body: formData,
			})
				.then(() => {
		...
		async getFilters() {
			return new Promise((resolve, reject) => {
				fetch('/index.php?option=com_emundus&controller=dashboard&task=getfilters&widget=' + this.selectedWidget.id)
					.then((response) => response.json())
					.then((data) => {
