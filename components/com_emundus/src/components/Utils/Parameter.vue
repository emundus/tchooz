<template>
  <div>
    ...
    <label v-if="parameter.hideLabel !== true"
      ...
      <span v-if="parameter.helptext && helpTextType === 'icon'"
    ...
    >
      ...
      <div v-else-if="parameter.type === 'radiobutton'">
        <fieldset data-toggle="radio_buttons"
          ...
          <div v-for="option in parameter.options" :key="option.value" class="fabrikgrid_radio">
            <input v-model="value" type="radio" class="fabrikinput" :class="parameter.hideRadio ? '!tw-hidden' : ''"
                   :name="paramName"
            ...
            <label :for="paramId + '_input_' + option.value">
              <span class="tw-flex tw-items-center tw-gap-2">
                <img v-if="option.img" :src="'/images/emundus/icons/'+option.img" :alt="option.altImg"
                     style="width: 16px">
                {{ translate(option.label) }}
              </span>
            </label>
      ...
      <div v-else-if="parameter.type === 'toggle'" class="tw-flex tw-items-center">
        ...
        <label :for="paramId + '_input'"
               class="tw-ml-2 !tw-mb-0 tw-font-bold tw-cursor-pointer tw-flex tw-items-center">
          <span v-if="parameter.iconLabel"
                class="material-symbols-outlined tw-mr-1 tw-text-neutral-900">{{ parameter.iconLabel }}</span>
          {{ translate(parameter.label) }}
        </label>
      </div>
