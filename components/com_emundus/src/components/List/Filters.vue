...
<template>
  <section id="tab-filters" class="tw-w-full">
    <div v-if="availableFilters.length > 0"
         :class="{'tw-mb-4': displayedFilters.length > 0}"
    >
    ...
    <div class="tw-grid tw-grid-cols-3 tw-gap-4">
      <div v-for="filter in defaultFilters" :key="currentTabKey + '-' + filter.key"
      ...
      <div v-for="filter in displayedFilters" :key="currentTabKey + '-' + filter.key"
           class="tw-flex tw-flex-col tw-gap-1"
      >
        <div class="tw-flex tw-items-center tw-justify-between">
          <label class="!tw-mb-0 tw-font-medium">
          ...
          <span class="material-icons-outlined tw-text-red-500 tw-cursor-pointer"
