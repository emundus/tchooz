    ...
    <div class="tw-flex tw-items-center tw-gap-2">
      <popover
        ...
        <ul class="tw-items-center tw-p-4 tw-list-none tw-m-0">
          <li v-for="exp in multipleExportsPopover"
              :key="exp.name"
              @click="onClickExport(exp)"
              class="tw-py-1.5 tw-px-2"
              :class="{
                      'tw-cursor-not-allowed tw-text-neutral-500': !(typeof exp.showon === 'undefined' || evaluateShowOn(exp.showon)),
                      'tw-cursor-pointer tw-text-base hover:tw-bg-neutral-300 hover:tw-rounded-coordinator': (typeof exp.showon !== 'undefined' && evaluateShowOn(exp.showon) || typeof exp.showon === 'undefined'),
                    }"
          >
            {{ translate(exp.label) }}
          </li>
