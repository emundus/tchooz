...
export default {
	...
	created() {
		this.contact = this.$props.item;

		this.fields.find((field) => field.param === 'files').value = this.contact.application_files.map((file) => file.id);
	},
	methods: {
		onClosePopup() {
			this.$emit('close');
		...
		updateContactFiles() {
			...
			if (filesField && filesField.value) {
				filesField.value.forEach((element) => {
					if (element.value) {
						filesIds.push(element.value);
					}
				});
			}

			crcService.updateContactFiles(this.contact.id, filesIds).then((response) => {
				if (response.status === true) {
					...
					Swal.fire({
						position: 'center',
						icon: 'success',
						title: Joomla.Text._('COM_EMUNDUS_ONBOARD_CRC_UPDATE_CONTACT_FILES_SUCCESS'),
	...
	<div>
		<div class="tw-pt-4">
			<div class="tw-mb-4 tw-flex tw-items-center tw-justify-between">
				<h2>
					{{ translate('COM_EMUNDUS_ONBOARD_CRC_UPDATE_CONTACT_FILES_ASSOCIATED_TITLE') }}
				</h2>
		...
		<div class="tw-mb-8 tw-mt-5 tw-flex tw-justify-between">
			<button class="tw-btn-cancel" @click="onClosePopup">
				{{ translate('COM_EMUNDUS_ONBOARD_CRC_UPDATE_CONTACT_FILES_ASSOCIATED_CANCEL') }}
			</button>
			<button class="tw-btn-primary" :disabled="disabledSubmit || !optionsReady" @click="updateContactFiles()">
				{{ translate('COM_EMUNDUS_ONBOARD_CRC_UPDATE_CONTACT_FILES_ASSOCIATED_CONFIRM') }}
			</button>
		</div>
