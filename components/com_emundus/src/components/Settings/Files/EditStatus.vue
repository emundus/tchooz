<template>
	<div class="tw-flex tw-flex-wrap tw-justify-start">
		<div class="tw-w-10/12">
			...
			<draggable handle=".handle" v-model="status" :class="'draggables-list'" @end="updateStatusOrder">
				<div
						...
						<div class="status-field">
							<div>
								<p
									...
									@keyup.enter="manageKeyup(statu)"
									@keydown="checkMaxlength"
								>
									{{ statu.label[actualLanguage] }}
								</p>
							</div>
	...
	methods: {
		...
		async updateStatus(status) {
			...
			if (newLabel.length > 0) {
				...
				await client()
					.post('index.php?option=com_emundus&controller=settings&task=updatestatus', formData, {
						headers: {
		...
		async updateStatusOrder() {
			...
			await client()
				.post('index.php?option=com_emundus&controller=settings&task=updatestatusorder', formData, {
					headers: {
						'Content-Type': 'multipart/form-data',
					},
				})
		...
		manageKeyup(status) {
			document.getElementById('status_label_' + status.step).textContent = document
				.getElementById('status_label_' + status.step)
				.textContent.trim();
