<template>
	<div
		...
		v-if="activeMenuItem.type === 'sectionComponent'"
		...
		<div @click="handleSectionComponent(activeMenuItem)" class="flex items-center justify-between cursor-pointer">
			<div class="flex">
				<h1 id="accordion-collapse-heading-1" class="user-select-none flex flex-row justify-between">
					<span :id="'Subtile'" class="text-2xl user-select-none">{{ translate(activeMenuItem.sectionTitle) }}</span>
				</h1>
			</div>
			<!-- The expand icon of the section wich rotate-->
			<span class="material-symbols-outlined scale-150 user-select-none" :id="'SubtitleArrow'" name="SubtitleArrows"
				>expand_more</span
			>
		</div>
