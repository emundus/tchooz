		...
		>
			...
			<div class="em-modal-content">
				...
				<transition name="fade" mode="out-in" v-if="selectedMenu">
					<EditArticle
						v-if="selectedMenu.type === 'article'"
						:key="currentMenu"
						:article_id="selectedMenu.id"
						:article_alias="selectedMenu.alias"
					...
					></EditArticle>
					<EditFooter
						v-else-if="selectedMenu.type === 'footer'"
						class="em-modal-component"
						@updateSaving="updateSaving"
						@updateLastSaving="updateLastSaving"
					></EditFooter>
...
export default {
	...
	created() {
		...
		client()
			.get('index.php?option=com_emundus&controller=settings&task=gethomearticle')
			.then((response) => {
				this.menus.push({
					type: 'article',
				...
				client()
					.get('index.php?option=com_emundus&controller=settings&task=getrgpdarticles')
					.then((response) => {
