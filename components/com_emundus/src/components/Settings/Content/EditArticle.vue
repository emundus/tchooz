...
export default {
	...
	methods: {
		async getArticle() {
			...
			await client()
				.get('index.php?option=com_emundus&controller=settings&task=getarticle', {
					params: params,
				})
				.then((response) => {
		...
		async saveContent() {
			...
			await client()
				.post(`index.php?option=com_emundus&controller=settings&task=updatearticle`, formData, {
					headers: {
		...
		async publishArticle() {
			...
			await client()
				.post(`index.php?option=com_emundus&controller=settings&task=publisharticle`, formData, {
					headers: {
						'Content-Type': 'multipart/form-data',
					},
				})
