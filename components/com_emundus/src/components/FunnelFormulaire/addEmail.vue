					...
					<div class="tw-flex tw-w-full tw-items-start tw-items-center tw-justify-between">
						<div>
							...
							<div class="tw-mb-2 tw-mt-2">
								...
								<span v-if="trigger.users.length == 0 && trigger.profile != 5 && trigger.profile != 6">{{
									translate('COM_EMUNDUS_ONBOARD_THE_CANDIDATE')
								}}</span>
...
export default {
	...
	methods: {
		...
		getTriggers() {
			axios
				.get('index.php?option=com_emundus&controller=email&task=gettriggersbyprogram&pid=' + this.prog)
				.then((response) => {
