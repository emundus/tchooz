...
<template>
  <div class="tw-relative">
    <div class="tw-absolute tw-bg-white tw-rounded-coordinator tw-p-3 tw-right-0 tw-top-6 tw-w-[25em] tw-shadow-standard tw-border tw-border-neutral-300">
        ...
        <div v-for="unread in unreadMessages" class="tw-flex tw-flex-col">
          <div class="tw-flex tw-items-center tw-gap-1 tw-flex-wrap tw-overflow-x-hidden">
            <label class="tw-flex tw-gap-1 tw-items-center tw-text-base tw-cursor-pointer tw-mb-0">
              <a type="button" class="tw-cursor-pointer tw-text-blue-500" @click="goToFile(unread.fnum)">{{ unread.fullname }}</a>
              <span @click="toggleConversation(unread.page)">{{ sentTranslation(unread) }}</span>
            </label>
            <span class="material-symbols-outlined tw-cursor-pointer" @click="toggleConversation(unread.page)" :class="{'tw-rotate-90': conversionOpened === unread.page}">chevron_right</span>
          </div>
          ...
          <div class="tw-mt-1 tw-pl-1 tw-border-s-2 tw-border-main-500" v-if="conversionOpened === unread.page">
            ...
            <div class="tw-mt-1 tw-mx-1 tw-flex tw-justify-between tw-items-center">
              <button type="button" class="tw-cursor-pointer tw-text-blue-500"
                      @click="closeChatroom(unread.fnum)">{{ translate('COM_EMUNDUS_MESSENGER_CLOSE_CHATROOM') }}
              </button>
