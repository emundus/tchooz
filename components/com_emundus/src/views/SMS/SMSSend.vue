  ...
  <div id="sms-send" class="tw-flex tw-flex-col tw-gap-2">
    ...
    <div class="tw-flex tw-flex-col tw-gap-1" v-if="templates.length > 0">
      ...
      <select v-model="selectedTemplate">
        ...
        <option v-for="template in displayedTemplates" :key="template.id" :value="template.id">{{ template.label[useGlobalStore().getShortLang] }}</option>
      </select>
    </div>
    ...
    <div class="tw-flex tw-justify-end">
      <button
          ...
          :disabled="message.length < 1 || fnums.length < 1"
          @click="sendSMS"
      >
...
export default {
  ...
  methods: {
    ...
    sendSMS() {
      if (this.fnums.length < 1 || this.message.length < 1) {
        Swal.fire({
          icon: 'error',
          title: this.fnums.length < 1 ? this.translate('COM_EMUNDUS_SMS_NO_RECIPIENTS') : this.translate('COM_EMUNDUS_SMS_NO_MESSAGE'),
  ...
  computed: {
    displayedTemplates() {
      return this.selectedCategory > 0 ? this.templates.filter(template => template.category_id === this.selectedCategory) : this.templates;
    }
  },
  watch: {
