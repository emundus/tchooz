<template>
  <div id="app">
    <div class="tw-flex tw-flex-wrap tw-shadow-standard tw-w-full tw-mb-6 tw-relative tw-rounded-coordinator-cards tw-min-h-[140px]">
      ...
      <div class="tw-z-0 tw-w-full tw-bg-profile-full tw-p-8 tw-m-0 tw-rounded-coordinator-cards tw-absolute tw-h-full"></div>
    </div>
...
export default {
  ...
  computed: {
    displayProfileName() {
      if(this.language == 1) {
        return this.profile_name !== '' ? this.translate('COM_EMUNDUS_DASHBOARD_AREA') + ' ' + this.profile_name.toLowerCase() : this.translate('COM_EMUNDUS_DASHBOARD_AREA') + ' ' + this.translate('COM_EMUNDUS_DASHBOARD_EMPTY_LABEL');
      } else {
        return this.profile_name !== '' ? this.profile_name.toUpperCase() + ' ' + this.translate('COM_EMUNDUS_DASHBOARD_AREA').toLowerCase() : this.translate('COM_EMUNDUS_DASHBOARD_EMPTY_LABEL') + ' ' + this.translate('COM_EMUNDUS_DASHBOARD_AREA');
      }
    },
  },
  watch: {
