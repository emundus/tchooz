...
<template>
  <div>
    ...
    <div class="tw-mt-7 tw-flex tw-flex-col tw-gap-6">
      ...
      >
        <Parameter
            ...
            :asyncAttributes="[fields.find(f => f.param === 'event_id')?.value, fields.find(f => f.param === 'user')?.value, field.param]"
    ...
    <div class="tw-flex tw-justify-between tw-mt-5 tw-mb-8">
      <button class="tw-btn-cancel"
              @click="onClosePopup">{{ translate('COM_EMUNDUS_ONBOARD_REGISTRANT_EDIT_CANCEL') }}</button>
      <button class="tw-btn-primary"
              :disabled="disabledSubmit"
              @click="editSlot()"
      >
        {{ translate('COM_EMUNDUS_ONBOARD_REGISTRANT_EDIT_CONFIRM') }}</button>
    </div>
