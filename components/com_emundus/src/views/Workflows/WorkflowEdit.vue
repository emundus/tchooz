    ...
    <div v-if="!loading">
      ...
      <div id="tabs-wrapper" class="tw-w-full tw-rounded-coordinator tw-p-6 tw-bg-white tw-border tw-border-neutral-300 tw-relative">
        <div v-if="activeTab.id === 'steps'" id="workflow-steps-wrapper" class="tw-flex tw-flex-col">
          <a class="tw-btn-primary tw-h-fit tw-w-fit tw-mb-4 tw-cursor-pointer" type="button" @click="addStep"> {{ translate('COM_EMUNDUS_WORKFLOW_ADD_STEP') }} </a>
                ...
                <div class="workflow-step-head tw-flex tw-flex-row tw-justify-between tw-gap-2">
                  ...
                  <popover>
                    <ul class="tw-list-none !tw-p-0">
                      <li class="archive-workflow-step tw-cursor-pointer tw-p-2 hover:tw-bg-neutral-300" @click="duplicateStep(step.id)">{{ translate('COM_EMUNDUS_ACTIONS_DUPLICATE') }}</li>
                      <li v-if="step.state == 1" class="archive-workflow-step tw-cursor-pointer tw-p-2 hover:tw-bg-neutral-300" @click="updateStepState(step.id, 0)">{{ translate('COM_EMUNDUS_ACTIONS_ARCHIVE') }}</li>
                      <li v-else class="archive-workflow-step tw-cursor-pointer tw-p-2 hover:tw-bg-neutral-300" @click="updateStepState(step.id, 1)">{{ translate('COM_EMUNDUS_ACTIONS_UNARCHIVE') }}</li>
                      <li class="delete-workflow-step tw-cursor-pointer tw-p-2 hover:tw-bg-neutral-300" @click="beforeDeleteStep(step.id)">{{ translate('COM_EMUNDUS_ACTIONS_DELETE') }}</li>
                    </ul>
                  </popover>
                </div>
        ...
        <div v-else-if="activeTab.id === 'programs'">
          ...
          <div class="tw-mt-4 tw-flex tw-flex-row tw-items-center tw-cursor-pointer">
            ...
            <label for="check-all" class="tw-cursor-pointer !tw-mb-0 tw-font-medium">{{ translate('COM_EMUNDUS_WORKFLOW_CHECK_ALL') }}</label>
          </div>
