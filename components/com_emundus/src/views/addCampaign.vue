		...
		<div>
			<form @submit.prevent="submit" v-if="ready" class="emundus-form fabrikForm">
				<div class="tw-flex tw-flex-col tw-gap-4">
					<div id="campaign-label-wrapper">
						<label for="campLabel" class="tw-font-medium"
							>{{ translate('COM_EMUNDUS_ONBOARD_ADDCAMP_CAMPNAME') }} <span class="tw-text-red-600">*</span></label
						>
						<input
							id="campLabel"
							type="text"
					...
					<div class="tw-grid tw-grid-cols-2 tw-gap-1.5">
						<div>
							<label for="startDate" class="tw-font-medium"
								>{{ translate('COM_EMUNDUS_ONBOARD_ADDCAMP_STARTDATE') }} <span class="tw-text-red-600">*</span></label
							>
							<DatePicker
						...
						<div>
							<div>
								<label for="endDate" class="tw-font-medium"
									>{{ translate('COM_EMUNDUS_ONBOARD_ADDCAMP_ENDDATE') }} <span class="tw-text-red-600">*</span></label
								>
								<DatePicker
					...
					<div>
						<label for="year" class="tw-font-medium"
							>{{ translate('COM_EMUNDUS_ONBOARD_ADDCAMP_PICKYEAR') }} <span class="tw-text-red-600">*</span></label
						>
						<div>
							<span class="tw-text-base tw-text-neutral-600">
				...
				<div class="tw-flex tw-flex-col tw-gap-4">
						...
						<div class="tw-flex tw-items-center">
							...
							>
								...
								>
									{{
										item.label &&
										item.label[actualLanguage] !== null &&
										typeof item.label[actualLanguage] != 'undefined'
											? item.label[actualLanguage]
											: item.label
									}}
								</option>
							</select>
							<button
								...
								:title="translate('COM_EMUNDUS_ONBOARD_ADDPROGRAM')"
								type="button"
								id="add-program"
					...
					<transition name="slide-fade">
						<div v-if="isHiddenProgram">
							<div>
								<div>
									<label for="prog_label" class="tw-font-medium"
										>{{ translate('COM_EMUNDUS_ONBOARD_PROGNAME') }} <span class="tw-text-red-600">*</span></label
									>
									<input
										type="text"
										id="prog_label"
				...
				<div class="tw-flex tw-flex-col tw-gap-4" id="campaign-form-container">
					<h2>
						{{ translate('COM_EMUNDUS_ONBOARD_ADDCAMP_FORM') }}
...
export default {
	...
	methods: {
		...
		submit() {
			...
			if (
				this.form.label[this.actualLanguage] === '' ||
				this.form.label[this.actualLanguage] == null ||
				typeof this.form.label[this.actualLanguage] === 'undefined'
			) {
			...
			if (
				...
				this.errors.year ||
				this.errors.limit_files_number ||
				this.errors.limit_status ||
				this.errors.progLabel ||
				this.errors.progCode ||
				this.errors.alias
			) {
				return 0;
			}
			...
			if (this.form.training !== '') {
				...
				this.programs.forEach((program) => {
					if (program.code === this.form.training) {
						programLang =
							program.language_ids != null && Array.isArray(program.language_ids) ? program.language_ids : [];
					}
				});
