...
export default {
					...
					if (response.status) {
						...
						this.messages.forEach((message) => {
							if (message.message_id === message_id) {
								...
								this.chatrooms
									.find((chatroom) => chatroom.ccid === this.currentChatroom.ccid)
									.messages.push({
	...
	computed: {
		...
		openedChatrooms() {
			...
			let chatroomsByMessage = this.chatrooms.filter((chatroom) =>
				chatroom.messages.some((message) => message.message.toLowerCase().includes(this.search.toLowerCase())),
			);
	...
	<div class="tw-h-full tw-overflow-hidden">
		<div v-if="!loading" class="tw-flex tw-h-full tw-flex-col">
			...
			<div
				...
				class="tw-mt-6 tw-h-full"
				:class="{ 'tw-grid': applicant == true }"
			...
			>
				...
				<div class="tw-flex tw-h-[95%] tw-flex-col tw-items-center tw-justify-between" v-if="applicant">
					<div class="tw-w-full">
						...
						<div class="tw-w-full">
							<div
								v-for="chatroom in openedChatrooms"
								:key="chatroom.ccid"
						...
						<div v-if="closedChatrooms.length > 0" class="tw-w-full">
							...
							>
								...
								<span
									class="material-symbols-outlined tw-transition-transform"
									:class="{ 'tw-rotate-90': showClosedChatroom }"
									>chevron_right</span
								>
							</div>
							...
							<div v-show="showClosedChatroom">
								<div
									v-for="chatroom in closedChatrooms"
									:key="chatroom.ccid"
									:class="currentChatroom && chatroom.ccid === currentChatroom.ccid ? 'tw-bg-neutral-300' : ''"
									class="tw-mt-3"
								>
