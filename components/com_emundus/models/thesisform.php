...
class EmundusModelThesisForm extends JModelForm
{
	...
	{
		// Get the form.
		$form = $this->loadForm('com_emundus.thesis', 'thesisform', array('control' => 'jform', 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	{
		$data = JFactory::getApplication()->getUserState('com_emundus.edit.thesis.data', array());
        if (empty($data)) {
            $data = $this->getData();
        }
    ...
    {
        $id = (!empty($data['id'])) ? $data['id'] : (int)$this->getState('thesis.id');
        if(JFactory::getUser()->authorise('core.delete', 'com_emundus.thesis.'.$id) !== true){
            JError::raiseError(403, JText::_('JERROR_ALERTNOAUTHOR'));
            return false;
        }
        $table = $this->getTable();
