...
class EmundusModelCampaign extends ListModel
{
	...
	public function publishCampaign($data, int $userId = 0): array
	{
		...
		if (!empty($data))
		{
			...
			{
				$more_form = $this->getCampaignMoreForm($data[0]);
				if (!empty($more_form))
				{
					$campaigns_to_remove = [];
					foreach ($data as $key => $val)
					{
						...
						$query->clear()
							...
							->where($this->_db->quoteName('campaign_id') . ' = ' . $this->_db->quote($val));
						$this->_db->setQuery($query);
						$campaign_more_row = $this->_db->loadResult();

						if(empty($campaign_more_row))
						{
							// Remove from ids to publish
							$campaigns_to_remove[] = $key;
						}
					}

					if(!empty($campaigns_to_remove))
					{
						foreach($campaigns_to_remove as $k)
						{
							unset($data[$k]);
						}
				...
				if(empty($data))
				{
					return ['success' => false, 'message' => 'COM_EMUNDUS_PUBLISH_ERROR_MORE_INFO_REQUIRED'];
				}
				else if(!empty($campaigns_to_remove))
				{
					$response['message'] = 'COM_EMUNDUS_CAMPAIGN_PUBLISHED_ERROR_SOME';
				}
			}
			...
			try
			{
				...
				$this->_db->setQuery($query);
				$response['success'] = $this->_db->execute();

				if ($response['success'])
				{
					if(empty($response['message']))
					{
						$response['message'] = 'COM_EMUNDUS_CAMPAIGN_PUBLISHED';
					}
					else {
			...
			catch (Exception $e)
			{
				...
				$response['message'] = 'COM_EMUNDUS_ERROR_OCCURED';
			}
		}
	...
	public function updateCampaign($data, $cid, int $user_id = 0)
	{
		...
		if (!empty($data) && !empty($cid))
		{
			...
			if($force_campaigns_more == 1)
			{
				$more_form = $this->getCampaignMoreForm($cid);
				if (!empty($more_form))
				{
					// Check if setup_campaigns_more row exist for this campaign
					$query->clear()
	...
	public function getCampaignMoreForm(?int $campaign_id = 0): array
	{
		$form = [];
		...
		if (!empty($form_id))
		{
			...
			$this->_db->setQuery($query);
			$elements = $this->_db->loadAssocList();
			...
			if (count($elements) > 0)
			{
				...
				if (!empty($campaign_id))
				{
					...
					$this->_db->setQuery($query);
					$form['row_id'] = $this->_db->loadResult();

					if (!empty($form['row_id']))
					{
						$form['url'] = 'index.php?option=com_fabrik&view=form&formid=' . $form_id . '&rowid=' . $form['row_id'] . '&tmpl=component&iframe=1';
					}
					else
					{
						$form['url'] = 'index.php?option=com_fabrik&view=form&formid=' . $form_id . '&rowid=0&tmpl=component&iframe=1&jos_emundus_setup_campaigns_more___campaign_id=' . $campaign_id . '&Itemid=0';
					}
				}
			}
		}
	...
	public function getCampaignMoreFormUrl(int $campaign_id): string
	{
		$form_url = '';

		if (!empty($campaign_id))
		{
			$form = $this->getCampaignMoreForm($campaign_id);

			if (!empty($form) && !empty($form['url']))
			{
				$form_url = $form['url'];
			}
		}
	...
	public function getCampaignMoreRowId(int $campaign_id): ?int
	{
		$query = $this->_db->getQuery(true);
