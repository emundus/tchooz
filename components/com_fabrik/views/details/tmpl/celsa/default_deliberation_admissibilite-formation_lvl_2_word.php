...
if (!empty($this->fnums))
{
    ...
    $query->clear()
        ->select('jee.fnum, ROUND(SUM(jee.epreuve_note) / SUM(jee.coeff),2) as moyenne_bel, jee.moyenne_l3, if(jepd.first_name is not null, jepd.first_name, jeu.firstname) as first_name, if(jepd.last_name is not null, jepd.last_name, jeu.lastname) as last_name')
	    ...
	    ->order('jee.moyenne_l3 DESC, moyenne_bel DESC');
    $db->setQuery($query);
	...
	foreach ($sorted_fnums as $key => $item)
	{
		$return['table']['rows'][] = array(
			...
			(!empty($item->moyenne_l3) ? $item->moyenne_l3 : $item->moyenne_bel),
		);
	}
}
