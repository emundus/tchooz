...
<?php if ($show_add_application && ($position_add_application == 0 || $position_add_application == 2) && $applicant_can_renew) : ?>
    ...
    <a id="add-application" class="btn btn-info btn-xs"
       href="<?= JText::_('MOD_EMUNDUS_APPLICATIONS_LINK_TO_INVOICES'); ?>">
        <span class="icon-plus-sign"><i
                    class="fas fa-file-invoice"></i> <?= JText::_('MOD_EMUNDUS_APPLICATIONS_MY_INVOICES'); ?></span>
    </a>
    <hr>
<?php endif; ?>
                ...
                <div class="row" id="row<?= $application->fnum; ?>">
                    <div class="col-md-12 main-page-application-title">
                        ...
                        <a href="<?= JRoute::_($first_page_url); ?>">
							<?= ($is_admission && $add_admission_prefix) ? JText::_('COM_EMUNDUS_INSCRIPTION') . ' - ' . JText::_('EXPEDITION_NUMBER') . date("d/m/Y", strtotime($application->campDateTime)) : JText::_('EXPEDITION_NUMBER') . date("d/m/Y", strtotime($application->campDateTime)); ?>
                        </a>

                    </div>

                    <div class="col-xs-12 col-md-8 main-page-file-info">
						...
						<?php if (in_array($application->status, [7, 8])) : ?>
                            <a id='issue' class="btn btn-xs" href="report?fnum=<?= $application->fnum; ?>"
                               title="<?= JText::_('MOD_EMUNDUS_APPLICATIONS_REPORT_AN_ISSUE'); ?>"><i
                                        class="exclamation triangle icon"></i> <?= JText::_('MOD_EMUNDUS_APPLICATIONS_REPORT_AN_ISSUE'); ?>
                            </a>
                            <a id='issue' class="btn btn-xs" href="bill?fnum=<?= $application->fnum; ?>"
                               title="<?= JText::_('BILL_REQUEST'); ?>"><i
                                        class="file alternate outline icon"></i> <?= JText::_('BILL_REQUEST'); ?></a>
						<?php endif; ?>
                    ...
                    <div class="col-xs-12 <?= ($show_state_files == 1) ? "col-md-3" : "col-md-4" ?> main-page-file-progress">
                        ...
                        <?= $application->value; ?>
							...
							<?php $delivery_info = modemundusApplicationsHelper::getDeliveryData($application->fnum);
							if (!empty($delivery_info)) :?>
                                <strong><?= JText::_('MOD_EMUNDUS_APPLICATIONS_TRACKING_LINK'); ?>:</strong>
                                <span class="label delivery-label">
                            <a title="<?= JText::_('MOD_EMUNDUS_APPLICATIONS_TRACKING_LINK_TOOLTIP'); ?>"
                               href="<?= $delivery_info->tracking_link; ?>"
                               target="_blank"><?= $delivery_info->tracking_number; ?></a>
                        </span>
...
<?php if ($show_add_application && $position_add_application > 0 && $applicant_can_renew) : ?>
    <a class="btn btn-success" href="href=<?= $cc_list_url; ?>"><span
