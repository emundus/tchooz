	...
	foreach($years as $year) {
		...
		foreach ($applications as $key => $application) {
			if ($application->year == $year) {
				$files_this_year[] = $application;
			}
		}
		if (count($files_this_year) > 0) {
			usort($files_this_year, function($a, $b) {
				return $a->wish_number > $b->wish_number;
			});

			?>
            <h2 id="group_application_tab_<?php echo $year ?>"><?php echo $year ?></h2>
            <div id="application-list-<?= $uniqid; ?>" class="<?= $moduleclass_sfx ?>">
				<?php foreach ($files_this_year as $a_index => $application) : ?>
					<?php
