    ...
    if ($helper->doINeedToPay($user->fnum) === true) {
        if ($helper->didIAlreadyPay($user->fnum) === false) {
            ...
            if (!empty($params['payment_methods'])) {
				...
				$wa->registerAndUseStyle('mod_emundus_payment', 'modules/mod_emundus_payment/assets/css/default.scss');
                ...
                if (sizeof($params['payment_methods']['payment_method']) > 1) {
                    ...
                    if (!empty($paymentStarted) && $helper->waitForValidation($paymentStarted)) {
                        ...
                        if (!empty($method_url_param)) {
							...
							$wa->registerAndUseStyle('mod_emundus_payment_selector', 'modules/mod_emundus_payment/assets/css/selector.scss');
                ...
                if (!empty($payment) && !empty($payment->product_id)) {
                    ...
                    if (!empty($product)) {
                        ...
                        switch($selected_payment_method) {
                            case 'flywire':
								$wa->registerAndUseStyle('mod_emundus_payment_flywire', 'modules/mod_emundus_payment/assets/css/flywire.scss');
