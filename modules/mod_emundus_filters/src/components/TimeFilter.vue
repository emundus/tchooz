<template>
	<div class="time-filter em-w-100 em-mb-8 em-border-radius-8 em-border-neutral-400 em-box-shadow em-white-bg em-p-8">
		<div class="em-flex-row em-flex-space-between">
			...
			<div>
				<span v-if="!filter.default" class="material-symbols-outlined em-red-600-color em-pointer" @click="$.emit('remove-filter')">close</span>
				<span v-if="opened === false" class="material-symbols-outlined em-pointer" @click="opened = !opened">keyboard_arrow_down</span>
				<span v-else class="material-symbols-outlined em-pointer" @click="opened = !opened">keyboard_arrow_up</span>
			</div>
		</div>
		...
		<section v-else class="default-filter-options em-mt-8">
			...
			<div v-if="filter.operator === 'between' || filter.operator === '!between'" class="em-mt-8">
				...
				<input class="em-mt-8" type="time" :id="filter.uid + '-end_date'" v-model="filter.value[1]"/>
			</div>
		</section>
	</div>
</template>
