    ...
    <div id="emundus-filters" class="em-w-100">
        <section id="filters-top-actions" class="em-mb-16">
            ...
            <div id="global-search-wrapper" style="position: relative;">
                <div id="global-search-values" ref="globalSearchValues"
                     class="em-border-radius-8 em-border-neutral-400 em-flex-row em-flex-wrap em-white-bg"
                    ...
                    <div v-if="globalSearch.length > 0" class="em-flex-row em-flex-wrap">
                        <div v-for="value in globalSearch" :key="value.value + '-' + value.scope"
                             class="global-search-tag em-flex-row em-box-shadow em-border-radius-8 em-border-neutral-400 em-w-auto em-mt-4 em-mb-4 em-ml-4 em-mr-4">
                            <span style="white-space: nowrap">{{ translatedScope(value.scope) }} : {{ value.value
                                }}</span>
                ...
                <ul id="select-scopes"
                    class="em-w-100 em-w-100 em-border-radius-8 em-white-bg em-border-neutral-400 em-box-shadow"
                    :class="{'hidden': currentGlobalSearch.length < 1}">
                    <li v-for="option in globalSearchScopes" :key="option.value"
                        @click="(e) => {this.onGlobalSearchChange(e, option.value)}"
                        class="em-pointer global-search-scope">
            ...
            <div id="save-filters-inputs-btns">
                <div id="save-filter-new-name"
                     class="em-flex-row em-flex-space-between em-border-radius-8 em-white-bg em-box-shadow em-w-100 em-p-16"
                     :class="{'hidden': !openSaveFilter}">
                    <input id="new-filter-name" ref="new-filter-name" type="text" class="em-flex-row"
                           v-model="newFilterName" :placeholder="translate('MOD_EMUNDUS_FILTERS_SAVE_FILTER_NAME')"
                           minlength="2" @keyup.enter="saveFilters" @focusout="onFocusOutNewFilter">
                    <span id="save-new-filter" class="material-symbols-outlined em-pointer"
                          :class="{'em-pointer em-dark-blue-500-color': newFilterName.length > 1}" @click="saveFilters">done</span>
                </div>
                <div v-if="registeredFilters.length > 0" id="registered-filters-wrapper" class="em-mt-8">
                    ...
                    <div class="em-flex-row em-flex-space-between">
                        <select id="registered-filters" class="em-w-100" v-model="selectedRegisteredFilter"
                                @change="onSelectRegisteredFilter">
                            <option value="0">{{ translate('MOD_EMUNDUS_FILTERS_PLEASE_SELECT') }}</option>
                            <option v-for="registeredFilter in registeredFilters" :key="registeredFilter.id"
                                    :value="registeredFilter.id">{{ registeredFilter.name }}
                            </option>
                        </select>
                        <span v-if="selectedRegisteredFilter > 0"
                              class="material-symbols-outlined em-red-600-color em-pointer"
                              @click="deleteRegisteredFilter">delete</span>
                    </div>
                </div>
            </div>
        </section>
        <section id="applied-filters">
            <div v-for="appliedFilter in appliedFilters" :key="appliedFilter.uid">
                <MultiSelect v-if="appliedFilter.type === 'select'" :filter="appliedFilter" :module-id="moduleId"
                             ...
                             @filter-changed="onFilterChanged"></MultiSelect>
                ...
                <DefaultFilter v-else :filter="appliedFilter" :module-id="moduleId" :type="appliedFilter.type"
                               class="em-w-100" @remove-filter="onRemoveFilter(appliedFilter)"
                               @filter-changed="onFilterChanged"></DefaultFilter>
            </div>
        </section>
        ...
        <section id="filters-bottom-actions">
            <button id="em-add-filter" class="tw-btn-cancel tw-w-full em-white-bg em-mt-16"
                    @click="openFilterOptions = !openFilterOptions">{{ translate('MOD_EMUNDUS_FILTERS_ADD_FILTER') }}
            </button>
