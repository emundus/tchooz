    ...
    <table class="table" width="100%">
        ...
        <tr>
			...
			if (!empty($this->options['show_cart_image']))
			{
				$row_count++;
				?>
                <th id="hikashop_cart_product_image_title"
                    class="hikashop_cart_product_image_title hikashop_cart_title"><?php
					echo Text::_('CART_PRODUCT_IMAGE');
					?></th>
			<?php } ?>
            <!-- EO IMAGE HEADER -->
            <!-- NAME HEADER -->
            <th id="hikashop_cart_product_name_title" class="hikashop_cart_product_name_title hikashop_cart_title"><?php
				echo Text::_('CART_PRODUCT_NAME');
				?></th>
            <!-- EO NAME HEADER -->
			...
			if (!empty($this->options['show_price']))
			{
				$row_count++;
				?>
                <th id="hikashop_cart_product_price_title"
                    class="hikashop_cart_product_price_title hikashop_cart_title"><?php
					echo Text::_('CART_PRODUCT_UNIT_PRICE');
					?></th>
				<?php
			}
			?>
            ...
            <!-- QUANTITY HEADER -->
            <th id="hikashop_cart_product_quantity_title"
                class="hikashop_cart_product_quantity_title hikashop_cart_title"><?php
				echo Text::_('PRODUCT_QUANTITY');
				?></th>
            <!-- EO QUANTITY HEADER -->
            <!-- TOTAL PRICE HEADER -->
			<?php
			if (!empty($this->options['show_price']))
			{
				$row_count++;
				?>
                <th id="hikashop_cart_product_total_title"
                    class="hikashop_cart_product_total_title hikashop_cart_title"><?php
					echo Text::_('CART_PRODUCT_TOTAL_PRICE');
					?></th>
				<?php
			}
			?>
		...
		foreach ($cart->products as $i => $product)
		{
            ...
            <tr class="row<?php echo $k; ?>">
				...
				{
					?>
                    <td data-title="<?php echo Text::_('CART_PRODUCT_IMAGE'); ?>"
                ...
                <!-- EO IMAGE -->
                <!-- NAME -->
                <td data-title="<?php echo Text::_('CART_PRODUCT_NAME'); ?>" class="hikashop_cart_product_name_value">
                    <p class="hikashop_cart_product_name"><?php
						...
						if (empty($this->options['status']) && $edit)
						{
							...
							echo ' ' . $popupHelper->display(
									...
									576, 480, 'title="' . Text::_('EDIT_THE_OPTIONS_OF_THE_PRODUCT') . '" class="edit_cart_product"', '', 'link'
								);
							...
							*/
                            ...
                            $product_characteristics = $m_payment->getHikashopProductVariantsAndCharacteristics($product->product_parent_id, $hikashop_user_id);
                            ?>
                                <select onchange="onChangeCharacteristic()"
                                        id="selected-product-characteristic"
                                        class="tw-p-2 tw-cursor-pointer"
                                        data-cart-id="<?=  $cart->cart_id ?>"
                                        data-cart-product-id="<?=  $product->cart_product_id ?>">
                                    <?php
                                    foreach ($product_characteristics as $product_characteristic) {
                                        ?>
                                        <option
                                            <?= $product_characteristic->product_id == $product->product_id ? 'selected' : '' ?>
                                            value="<?php echo $product_characteristic->product_id; ?>"
				...
				if (hikashop_level(1) && !empty($this->extraFields['product']))
				{
					foreach ($this->extraFields['product'] as $field)
					{
						$namekey = $field->field_namekey;
						?>
                        <td data-title="<?php echo $this->fieldClass->trans($field->field_realname); ?>"
				...
				{
					?>
                    <td data-title="<?php echo Text::_('CART_PRODUCT_UNIT_PRICE'); ?>"
                        class="hikashop_cart_product_price_value"><?php
						echo $this->getDisplayProductPrice($product, true);

						if (HIKASHOP_RESPONSIVE)
						{
							?><span class="visible-phone"><?php echo Text::_('PER_UNIT'); ?></span><?php
						}
						?></td>
					<?php
				}
                ...
                <!-- EO UNIT PRICE -->
                <!-- QUANTITY -->
                <td data-title="<?php echo Text::_('PRODUCT_QUANTITY'); ?>"
				...
				{
					?>
                    <td data-title="<?php echo Text::_('CART_PRODUCT_TOTAL_PRICE'); ?>"
		...
		if (!empty($this->options['show_price']) && (!empty($cart->coupon) || !empty($cart->shipping) || !empty($cart->additional) || $taxes > 0))
		{
            ...
            <!-- SEPARATOR ROW -->
            <tr class="margin">
                <td colspan="<?php echo $row_count; ?>" class="hikashop_cart_empty_footer"></td>
            </tr>
            <!-- EO SEPARATOR ROW -->
            <!-- SUBTOTAL ROW -->
            <tr>
                ...
                <td id="hikashop_checkout_cart_total2_title"
                    class="hikashop_cart_subtotal_title hikashop_cart_title"><?php
					echo Text::_('SUBTOTAL');
					?></td>
                <td class="hikashop_cart_subtotal_value" data-title="<?php echo Text::_('SUBTOTAL'); ?>">
		...
		if (!empty($this->options['show_price']) && !empty($cart->coupon) && !empty($this->options['show_coupon']))
		{
            ...
            <tr>
                ...
                <td id="hikashop_checkout_cart_coupon_title"
                    class="hikashop_cart_coupon_title hikashop_cart_title"><?php
					echo Text::_('HIKASHOP_COUPON');
					?></td>
                <td class="hikashop_cart_coupon_value" data-title="<?php echo Text::_('HIKASHOP_COUPON'); ?>">
		...
		if (!empty($this->options['show_price']) && !empty($cart->shipping) && !empty($this->options['show_shipping']))
		{
            ...
            <tr>
                <td colspan="<?php echo $row_count - 2; ?>" class="hikashop_cart_empty_footer"></td>
                <td id="hikashop_checkout_cart_shipping_title"
                    class="hikashop_cart_shipping_title hikashop_cart_title"><?php
					echo Text::_('HIKASHOP_SHIPPING');
					?></td>
                <td class="hikashop_cart_shipping_value" data-title="<?php echo Text::_('HIKASHOP_SHIPPING'); ?>">
		...
		if (!empty($cart->additional))
		{
			...
			foreach ($cart->additional as $k => $additional)
			{
                ...
                <tr id="hikashop_checkout_cart_additional_<?php echo str_replace(' ', '_', $k); ?>_line">
                    <td colspan="<?php echo $row_count - 2; ?>" class="hikashop_cart_empty_footer"></td>
                    <td id="hikashop_checkout_cart_additional_<?php echo str_replace(' ', '_', $k); ?>_title"
                        class="hikashop_cart_additional_title hikashop_cart_title"><?php
						echo Text::_($additional->name);
						if (empty($this->options['status']) && !empty($additional->deletable))
						{
							...
							?>
                            <a href="#removeAdditional"
                               onclick="return window.checkout.removeAdditional(<?php echo $this->step; ?>,<?php echo $this->module_position; ?>,'<?php echo $k; ?>');" <?php echo $attribs; ?>><i
                                        class="fas fa-trash"></i></a>
							<?php
						}
						?></td>
                    <td class="hikashop_cart_additional_value" data-title="<?php echo Text::_($additional->name); ?>">
...
?>
			...
			else
			{
                ...
                <tr>
                    ...
                    <td id="hikashop_checkout_cart_tax_title" class="hikashop_cart_tax_title hikashop_cart_title"><?php
						echo Text::_('TAXES');
						?></td>
                    <td class="hikashop_cart_tax_value" data-title="<?php echo Text::_('TAXES'); ?>">
				<span class="hikashop_checkout_cart_taxes"><?php
		...
		if (!empty($this->options['show_price']) && !empty($cart->payment) && $cart->payment->payment_price != 0 && !empty($this->options['show_payment']))
		{
            ...
            <tr>
                <td colspan="<?php echo $row_count - 2; ?>" class="hikashop_cart_empty_footer"></td>
                <td id="hikashop_checkout_cart_payment_title"
                    class="hikashop_cart_payment_title hikashop_cart_title"><?php
					echo Text::_('HIKASHOP_PAYMENT');
					?></td>
                <td class="hikashop_cart_payment_value" data-title="<?php echo Text::_('HIKASHOP_PAYMENT'); ?>">
		...
		if (!empty($this->options['show_price']))
		{
            ...
            <tr>
                ...
                <td id="hikashop_checkout_cart_final_total_title" class="hikashop_cart_total_title hikashop_cart_title">
                    <b><?php
						echo Text::_('HIKASHOP_TOTAL');
						?></b></td>
                <td class="hikashop_cart_total_value" data-title="<?php echo Text::_('HIKASHOP_TOTAL'); ?>">
...
if (!empty($this->extraData[$this->module_position]) && !empty($this->extraData[$this->module_position]->bottom))
{
...
if (false)
{
	?>
    <noscript>
        <input id="hikashop_checkout_cart_quantity_button" class="btn button" type="submit" name="refresh"
               value="<?php echo Text::_('REFRESH_CART'); ?>"/>
    </noscript>
	<?php
}
...
if (empty($this->ajax))
{
    ...
    <script type="text/javascript">
        ...
        window.checkout.refreshCart = function (step, id) {
            return window.checkout.refreshBlock('cart', step, id);
        };
        window.checkout.submitCart = function (step, id) {
            return window.checkout.submitBlock('cart', step, id);
        };
        ...
        function onChangeCharacteristic() {
            const selector = document.getElementById('selected-product-characteristic');

            if (selector) {
                const selectedOption = selector.options[selector.selectedIndex];

                const selected_product_characteristic = selector.value;
                const hikashop_product_characteristic = selectedOption.dataset.characteristicId;
                const hikashop_product_parent_characteristic = selectedOption.dataset.parentcharacteristicid;
                const cart_id = selector.dataset.cartId;
                const cart_product_id = selector.dataset.cartProductId;
                const formData = new FormData();
                formData.append('new_product_id', selected_product_characteristic);
                formData.append('hikashop_product_characteristic[' + hikashop_product_parent_characteristic + ']', hikashop_product_characteristic);
                formData.append('option', 'com_hikashop');
                formData.append('ctrl', 'cart');
                formData.append('task', 'product_save');
                formData.append('cart_product_id', cart_product_id);
                formData.append('cart_id', cart_id);
                formData.append('tmpl', 'component');
                formData.append('<?=  hikashop_getFormToken() ?>', 1);

                fetch('/component/hikashop/cart/product_save', {
                    method: 'POST',
                    body: formData
                }).then((response) => {
