...
<form action="<?php echo hikashop_completeLink('cart&task=product_save'); ?>" method="post"
    ...
    <fieldset class="hkform-horizontal">
        ...
        <div class="hkform-group control-group hikashop_item_product_name_line" id="hikashop_item_product_name">
            ...
            <div class="<?php echo $inputcolumnclass; ?> hikashop_item_product_name_text"
                 style="height:<?php echo $thumbnail_y; ?>px;">
                <div>
                    <p class="hikashop_item_product_name_p"><?php echo Text::_($this->product->product_name . $text); ?></p>
                    <p class="hikashop_item_product_qty"><?php echo Text::sprintf('WITH_A_QUANTITY_OF_X', $this->product->cart_product_quantity); ?></p>
                </div>
            </div>
        </div>
        <!-- EO NAME -->
		...
		if (!empty($this->parentProduct->characteristics))
		{
			...
			$list = '[\'' . implode('\',\'', $ids) . '\']';
			?>
            <input id="new_product_id" name="new_product_id" type="hidden"
			...
			case 'dropdown':
				...
				foreach ($this->characteristics as $characteristic)
				{
					...
					if ($characteristics_dynamic_display && $count > 1)
					{
						...
						{
							$this->values[] = JHTML::_('select.option', '', Text::_('PLEASE_SELECT'));
						}
					}
            ...
            <script>
                ...
                function initVariants() {
                    var allRows = document.querySelectorAll('div[data-characrow]'), first = true,
                        qtyArea = document.getElementById('buttons_right'),
                        altArea = document.getElementById('buttons_right_alt');
                ...
                function hikashopVariantSelected(obj) {
                    ...
                    if (!obj)
                        return true;
                    var pos = obj.getAttribute('data-characteristic'), last = obj.getAttribute('data-last'),
                        otherRow = null,
                        qtyArea = document.getElementById('buttons_right'),
                        altArea = document.getElementById('buttons_right_alt'),
		...
		if (!empty($this->options))
		{
			...
			foreach ($this->options as $optionElement)
			{
				...
				if (empty($optionElement->variants))
				{
					...
					{
						$option_values[] = JHTML::_('select.option', 0, Text::_('HIKASHOP_NO'));
					}
					$text = Text::_('HIKASHOP_YES');
                ...
                <div class="hkform-group control-group hikashop_option_<?php echo $id; ?>_line"
                     id="hikashop_option_<?php echo $id; ?>">
                    <div class="<?php echo $labelcolumnclass; ?> hkcontrol-label">
                    ...
                    <div class="<?php echo $inputcolumnclass; ?>"><?php
						...
						if (!empty($this->show_option_quantity) && $select != 'radio')
						{
							...
							{
								?>
                                <input type="text" class="hikashop_product_quantity_field"
                                       id="hikashop_product_option_qty_<?php echo $i; ?>"
                                       name="hikashop_product_option_qty[<?php echo $i; ?>]"
                                       onchange="hikaProductOptions.change();" value="1"/>
								<?php
							}
							else
							{
	...
	';
		...
		if (!empty($this->itemFields))
		{
			...
			foreach ($this->itemFields as $fieldName => $oneExtraField)
			{
				...
				?>
                <div class="hkform-group control-group hikashop_item_<?php echo $oneExtraField->field_namekey; ?>_line"
                     id="hikashop_item_<?php echo $oneExtraField->field_namekey; ?>">
					<?php
