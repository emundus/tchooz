...
function pdf_admission($user_id, $fnum = null, $output = true, $name = null, $options = null)
{
	...
	try {
		...
		$title = !$anonymize_data ? strtoupper(@$item->lastname) . ' ' . @$item->firstname : $config->get('sitename');
		$htmldata = '<html>
				<head>
				  <title>'.$title.'</title>
	...
	// get decision
	$data = (!$anonymize_data) ? EmundusHelperFiles::getAdmission('html', $fnum, $item->firstname.' '.strtoupper($item->lastname)) : EmundusHelperFiles::getAdmission('html', $fnum);

	foreach ($data as  $evals) {
		foreach ($evals as $html) {
			$htmldata .= $html;
		}
	}
