...
class FilesHelperTest extends UnitTestCase
{
	...
	public function test_moduleBuildWhere()
	{
		...
		$this->assertSame(' AND esc.published > 0 AND (jecc.applicant_id LIKE \'%test%\' OR jecc.fnum LIKE \'%test%\' OR ((u.username LIKE \'%test%\' OR eu.firstname LIKE \'%test%\' OR eu.lastname LIKE \'%test%\' OR u.email LIKE \'%test%\') AND eu.is_anonym <> 1)) AND jecc.published = \'1\'', $where['q'], 'Build where with filters returns correct string');
