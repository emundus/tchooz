...
class InternalReferenceServiceTest extends UnitTestCase
{
	...
	/**
	 * @covers \Tchooz\Services\Reference\InternalReferenceService::generateShortReferences
	 * Bulk generation used by the migration postflight: one map id => short reference,
	 * all unique, all valid, and consistent with the deterministic single-file generation.
	 */
	public function testGenerateShortReferences(): void
	{
		$applicationFileRepository = new ApplicationFileRepository();

		$file1 = $applicationFileRepository->getByFnum($this->dataset['fnum']);
		$fnum2 = $this->h_dataset->createSampleFile($this->dataset['campaign'], $this->dataset['applicant']);
		$file2 = $applicationFileRepository->getByFnum($fnum2);

		$rows = [
			(object) ['id' => $file1->getId(), 'campaign_id' => $file1->getCampaignId(), 'applicant_id' => $file1->getUser()->id],
			(object) ['id' => $file2->getId(), 'campaign_id' => $file2->getCampaignId(), 'applicant_id' => $file2->getUser()->id],
		];

		$references = $this->internalReferenceService->generateShortReferences($rows);

		$this->assertCount(2, $references, 'Should generate one short reference per file');
		$this->assertArrayHasKey($file1->getId(), $references, 'References should be keyed by application file id');
		$this->assertArrayHasKey($file2->getId(), $references, 'References should be keyed by application file id');

		foreach ($references as $reference) {
			$this->assertEquals(5, strlen($reference), 'The short reference should be 5 characters long');
			$this->assertDoesNotMatchRegularExpression('/[O0IL]/', $reference, 'The short reference should not contain O, 0, I, 1 or L');
		}

		$this->assertCount(2, array_unique($references), 'Generated short references should be unique across the batch');

		// The deterministic suffix (chars 2-5) must match the single-file generation for the same file
		$single = $this->internalReferenceService->generateShortReference($file1);
		$this->assertEquals(substr($single, 1), substr($references[$file1->getId()], 1), 'Bulk generation should reuse the deterministic suffix');
	}

	/**
	 * @covers \Tchooz\Services\Reference\InternalReferenceService::generateShortReferences
	 * A reference already stored must never be regenerated by the bulk pass.
	 */
	public function testGenerateShortReferencesAvoidsExisting(): void
	{
		$applicationFileRepository = new ApplicationFileRepository();
		$file                      = $applicationFileRepository->getByFnum($this->dataset['fnum']);

		// Pre-compute the deterministic reference and persist it so the batch must fall back.
		$deterministic = $this->internalReferenceService->generateShortReference($file);
		$file->setShortReference($deterministic);
		$applicationFileRepository->flush($file, $this->dataset['coordinator']);

		$rows = [
			(object) ['id' => $file->getId(), 'campaign_id' => $file->getCampaignId(), 'applicant_id' => $file->getUser()->id],
		];

		$references = $this->internalReferenceService->generateShortReferences($rows);

		$this->assertNotEquals($deterministic, $references[$file->getId()], 'Bulk generation must not reuse an existing short reference');
		$this->assertEquals(5, strlen($references[$file->getId()]));
	}
	...
	public function testGenerateDeterministicSuffix(): void
	{
		...
		$suffix1 = $this->invokeMethod($service, 'generateDeterministicSuffix', [123, 456, 789, $digits, $alphabet]);
		$suffix2 = $this->invokeMethod($service, 'generateDeterministicSuffix', [123, 456, 789, $digits, $alphabet]);
		...
		$suffix3 = $this->invokeMethod($service, 'generateDeterministicSuffix', [123, 456, 790, $digits, $alphabet]);
